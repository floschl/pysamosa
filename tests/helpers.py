...
def plot_retrack_result(l1b_data, l2_data, res_fit, model_params, *,
    ...
    fg_epoch = l1b_data['dynamic_fg_epoch'] if 'dynamic_fg_epoch' in l1b_data else np.argmax(
        l1b_data['wf'])
    if wf_sets is not None:
        fg_epoch *= wf_sets.internal_oversampling_factor

    wf_len = len(res_fit['wf'])
    wf_sets = wf_sets if wf_sets is not None else WaveformSettings.get_default_src_type(
        l1b_srctype)
    ...
    l2_epoch_ns = l2_data['epoch_ns'] if 'epoch_ns' in l2_data and not np.isnan(
        l2_data['epoch_ns']) else np.nan
    try:
    ...
    if show_l2_model:
        plot_data.append({'name': 'y_meas_model',
                          'wf': wf_l2_model,
    ...
    for d in plot_data:
        _ax.plot(d['wf'], linewidth=0.7, label='\n'.join(wrap(
            ...
            f"ssh_unc={d['ssh_unc']:.3f}m "            # f"Pu={d['Pu_W']:.2e}W"
            # f"nu={d['nu']:.2e}"
            , width=legend_len)))
    ...
    if 'interference_mask' in res_fit and not np.allclose(
        ...
        _ax.plot(res_fit['interference_mask'], linewidth=0.7,
                 label='interference reference waveform')

    if show_leading_edge and res_fit['le_inds'] is not None:
        le_inds = res_fit['le_inds']
        _ax.axvspan(le_inds[0], le_inds[-1], alpha=0.5,
                    color='lightblue', label='leading edge')
...
def plot_rip_result(
        l1b_data_single: dict,
        ripa: RipAnalyser,
        rip_params: RIPParameters,
        ax=None):
    if ax is not None:
        _ax = ax
    else:
    ...
    _ax.plot(
        ripa.doppler_beam_inds,
        rip_params.rip_az_fitted,
        '+',
        label='fitted gaussian')
    _ax.axvline(
        rip_params.pitch_mispoint_look,
        color='red',
        linestyle='dashed',
        linewidth=0.5)
    ...
    _ax.plot(
        ...
        0.5 *
        rip_params.amplitude_fitted_norm *
        np.ones(2),
        color='red',
        linestyle='dashed',
        linewidth=0.5,
    ...
    text_str = '\n'.join([f'{k} = {v:.3e}' for k, v in rip_params.dict(
...
def get_l1b_data_single_from_l1b_fixture(
    *,
    ...
    model_params_single = get_model_param_obj_from_l1b_data(
        l1b_data, ind=n_pre)
