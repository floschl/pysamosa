...
from pysamosa.common_types import RetrackerProcessorSettings, WaveformSettings, ModelSettings, \
    RetrackerSettings, SensorSettings, RetrackerBaseType, L1bSourceType, SENSOR_SETS_DEFAULT_S3
...
def plot_retrack_result(l1b_data, l2_data, res_fit, model_params, *,
    ...
    fg_epoch = l1b_data['dynamic_fg_epoch'] if 'dynamic_fg_epoch' in l1b_data else np.argmax(l1b_data['wf'])
    if wf_sets is not None:
        fg_epoch *= wf_sets.internal_oversampling_factor

    wf_len = len(res_fit['wf'])
    wf_sets = wf_sets if wf_sets is not None else WaveformSettings.get_default_src_type(l1b_srctype)

    if retrack_sets and retrack_sets.normalise_wf_by_fg_region:
        wf_meas = res_fit['wf'] / SamosaRetracker.get_wf_max(res_fit['wf'], retrack_sets=retrack_sets, fg_epoch=fg_epoch)
    else:
    ...
    l2_epoch_ns = l2_data['epoch_ns'] if 'epoch_ns' in l2_data and not np.isnan(l2_data['epoch_ns']) else np.nan
    try:
        wf_l2_model = sm.get_waveform_multilook(Pu=Pu_W_l2, Hs=l2_data['swh'], t0_ns=l2_epoch_ns, model_params=model_params)
    ...
    tracker_range_m = l2_data['tracker_range_m'] if 'tracker_range_m' in l2_data else l1b_data['tracker_range_m']
    ...
    plot_data = [
        {'name': 'y_l2', 'wf': wf_meas, 'misfit': l2_data.get('misfit', np.nan), 'l2norm': np.nan, 'n_iter': n_iter,
         'swh': l2_data['swh'], 'epoch_ns': float(l2_epoch_ns), 'range': range_l2, 'ssh_unc': ssh_unc_l2, 'Pu_W': Pu_W_l2, 'nu': np.nan},
        {'name': 'y_retrack', 'wf': res_fit['wf_opt'], 'misfit': res_fit['misfit'], 'misfit_selective': res_fit['misfit_selective'],
         'misfit_le': res_fit['misfit_le'] if 'misfit_le' in res_fit else np.nan,
         'l2norm': res_fit['l2norm'],
         'n_iter': res_fit['n_iter'], 'swh': res_fit['swh'], 'epoch_ns': res_fit['epoch_ns'], 'range': range_fit, 'ssh_unc': ssh_unc_fit, 'Pu_W': Pu_W_fit, 'nu': res_fit['nu']},
    ]

    if show_l2_model:
        plot_data.append(
            {'name': 'y_meas_model', 'wf': wf_l2_model, 'misfit': misfit_l2_model, 'l2norm': wf_l2_model_l2norm, 'n_iter': np.nan,
             'swh': l2_data['swh'], 'epoch_ns': l2_epoch_ns, 'range': range_l2, 'ssh_unc': ssh_unc_l2, 'Pu_W': Pu_W_l2, 'nu': np.nan})

    legend_len = 90 if legend_wrap_len is None else legend_wrap_len
    for d in plot_data:
        _ax.plot(d['wf'], linewidth=0.7, label='\n'.join(wrap(
            ...
            f"ssh_unc={d['ssh_unc']:.3f}m "\
            # f"Pu={d['Pu_W']:.2e}W"
            # f"nu={d['nu']:.2e}"
            , width=legend_len)))

    # plot initial first-guess epoch
    fg_epoch_style = {'linestyle': '--', 'linewidth': 1, 'color': 'grey', 'label': 'Dynamic First-Guess Epoch (DFGE)'}
    _ax.axvline(fg_epoch, **fg_epoch_style)

    if 'interference_inds' in res_fit:
        distorted_regions = consecutive_regions_from_ind_list(res_fit['interference_inds'])
        for br in distorted_regions:
            _ax.axvspan(br[0], br[-1], alpha=0.5, color='red')

    if 'interference_mask' in res_fit and not np.allclose(res_fit['interference_mask'], 1.0):
        _ax.plot(res_fit['interference_mask'], linewidth=0.7 ,label='interference reference waveform')

    if show_leading_edge and res_fit['le_inds'] is not None:
        le_inds = res_fit['le_inds']
        _ax.axvspan(le_inds[0], le_inds[-1], alpha=0.5, color='lightblue', label='leading edge')

    if retrack_sets is not None and retrack_sets.subwaveform_mode:
        _ax.axvspan(res_fit['max_le_gate'] + retrack_sets.subwaveform_n_gates_after_le, wf_len, alpha=0.5, color='gray')
    ...
    legend_kwargs_default = {
        ...
        'mode': 'expand',
        **{'prop': {'size': fontsize_textbox}, 'labelspacing': 0.50, 'borderaxespad': 0.2}
    }
    ...
    _ax.text(x=0.95, y=0.95, s='\n'.join(text_str_elems), ha='right', va='center', transform=_ax.transAxes, fontsize=fontsize_textboxes,
...
def plot_rip_result(l1b_data_single: dict, ripa: RipAnalyser, rip_params: RIPParameters, ax=None):
    if ax is not None:
        _ax = ax
    else:
        _ax = plt.gca()
    ...
    _ax.plot(ripa.doppler_beam_inds, rip_params.rip_az_fitted, '+', label='fitted gaussian')
    _ax.axvline(rip_params.pitch_mispoint_look, color='red', linestyle='dashed', linewidth=0.5)
    _ax.legend(fontsize=7, loc='upper left')

    _ax.plot(rip_params.halfpower_looks, 0.5 * rip_params.amplitude_fitted_norm * np.ones(2), color='red', linestyle='dashed', linewidth=0.5, label='3db aperture')
    ...
    text_str = '\n'.join([f'{k} = {v:.3e}' for k, v in rip_params.dict().items() if isinstance(v, numbers.Number)])
    _ax.text(x=0.98, y=0.05, s=text_str, ha='right', va='bottom', transform=_ax.transAxes, fontsize=fontsize_textboxes,
    ...
    _ax.text(x=0.98, y=0.95, s='\n'.join(text_str_elems), ha='right', va='top', transform=_ax.transAxes, fontsize=fontsize_textboxes,
            bbox=dict(pad=0.4, fc="white", alpha=0.8))
...
def get_l1b_data_single_from_l1b_fixture(*, l1b_fixture, ind, rp_sets : RetrackerProcessorSettings, file_id):
