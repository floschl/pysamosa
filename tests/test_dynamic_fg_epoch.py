...
from pysamosa.retracker_helpers import get_tracker_shift_n_samps, get_pointwise_product, shift_waveform_by_n, \
    get_dynamic_first_guess_epochs, get_inds_sliding_window, gen_sliding_window_from_seq


@pytest.mark.parametrize("n_offset, n_inds", [
    (25800,100),
])
def test_get_tracker_shift_n_samps(n_offset, n_inds, s3_eum_l1b):
    ...
    n_shifts = get_tracker_shift_n_samps(tracker_range_m=l1b_data['tracker_range_m'][:20], alt_m=l1b_data['alt_m'][:20], t0_sampling_gate_ns=SENSOR_SETS_DEFAULT_S3.B_r_Hz)
...
def test_get_dynamic_first_guess_epochs_sam_coast(n_offset, n_inds, s3_eum_l1b):
    ...
    fg_epoch = get_dynamic_first_guess_epochs(wfs=l1b_data['wf'], tracker_range=l1b_data['tracker_range_m'],
                                              alt_m=l1b_data['alt_m'], bu_bw_Hz=SENSOR_SETS_DEFAULT_S3.B_r_Hz,
...
def test_get_dynamic_first_guess_epochs_single_multcore(n_offset, n_inds, s3_eum_l1b):
    ...
    fg_epoch_multcore = get_dynamic_first_guess_epochs(wfs=l1b_data['wf'], tracker_range=l1b_data['tracker_range_m'],
                                                       alt_m=l1b_data['alt_m'], bu_bw_Hz=SENSOR_SETS_DEFAULT_S3.B_r_Hz,
                                                       fg_epoch_adjacent_meas=40,
                                                       n_procs=12,
                                                       )
...
def test_get_dynamic_first_guess_epochs_samplus(n_offset, n_inds, s3_eum_l1b):
    ...
    fg_epoch = get_dynamic_first_guess_epochs(wfs=l1b_data['wf'], tracker_range=l1b_data['tracker_range_m'], alt_m=l1b_data['alt_m'],
...
def test_get_pointwise_product():
    ...
    assert all(get_pointwise_product(X) == np.array([0, 4, 16, 36, 64, 100, 144, 196, 256, 324]))
