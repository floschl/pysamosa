...
def test_get_tracker_shift_n_samps(n_offset, n_inds, s3_eum_l1b):
    ...
    n_shifts = get_tracker_shift_n_samps(
        tracker_range_m=l1b_data['tracker_range_m'][:20], alt_m=l1b_data['alt_m'][:20], t0_sampling_gate_ns=SENSOR_SETS_DEFAULT_S3.B_r_Hz)
...
def test_get_dynamic_first_guess_epochs_sam_coast(
        n_offset, n_inds, s3_eum_l1b):
    l1b_data = s3_eum_l1b(n_offset=n_offset, n_inds=n_inds, file_id='s3_0')
...
def test_get_dynamic_first_guess_epochs_single_multcore(
        n_offset, n_inds, s3_eum_l1b):
    l1b_data = s3_eum_l1b(n_offset=n_offset, n_inds=n_inds, file_id='s3_0')
...
def test_genslidingwindow():
    ...
    for i, (wnd, center_sample) in enumerate(
            gen_sliding_window_from_seq(x, n)):
...
def test_get_pointwise_product():
    ...
    assert all(get_pointwise_product(X) == np.array(
