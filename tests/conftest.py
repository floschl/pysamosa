...
from pysamosa.data_access import _read_dataset_vars_from_ds, data_vars_s3, data_vars_s6, data_vars_cs, data_vars_dart
...
class DatasetReader():
    def __init__(self, src, data_vars, group='', file_level=None):
        ...
        if src.is_dir():
            self.ncfiles = sorted(list(src.rglob('*.nc')), key=lambda f: str(f.name).replace('RES_', '')[16:31])
        else:
    ...
    def __call__(self, *, n_offset=0, n_inds=1, file_id=None):
        ...
        return _read_dataset_vars_from_ds(nc_filename=ncfile, data_var_names=self.data_vars, n_offset=n_offset,
                                      n_inds=n_inds, group=self.group)

    def get_nc_filename(self, file_id=None):
        try:
            if file_id and isinstance(self.ncfiles, list):
                file_pattern = file_id_mappings[file_id] if not self.file_level else file_id_mappings[file_id].replace('1B', self.file_level.value.ljust(2,'_'))
                nc_filename = [nc for nc in self.ncfiles
...
@pytest.fixture
def s6_eum_l1b():
    return DatasetReader(src=S6_DATA_DIR / 'l1b', data_vars=data_vars_s6['l1b'], group='data_20/ku')

@pytest.fixture
def s6_eum_l2():
    return DatasetReader(src=S6_DATA_DIR / 'l2', data_vars=data_vars_s6['l2'], group='data_20/ku', file_level=FileLevel.L2)

@pytest.fixture
def s6_dart_l1b():
    return DatasetReader(src=S6_DATA_DIR / 'l1b', data_vars=data_vars_dart['l1b'], group='')

@pytest.fixture
def s6_dart_l2():
    return DatasetReader(src=FFSAR_DATA_DIR / 'l2', data_vars=data_vars_dart['l2'], group='', file_level=FileLevel.L2)
...
@pytest.fixture
def cs_eum_l2():
    return DatasetReader(src=CS_DATA_DIR / 'l2', data_vars=data_vars_cs['l2'], file_level=FileLevel.L2)

@pytest.fixture
def dataset_generic_l1b():
    def _dataset_reader(ncfile, data_vars):
        grps = [f'{g}/ku' for g in list(netCDF4.Dataset(ncfile).groups) if 'data_20' in g]
        return DatasetReader(src=ncfile, data_vars=data_vars, file_level=FileLevel.L1B, group=grps[0] if len(grps) else '')
...
def dataset_generic_l2():
    def _dataset_reader(ncfile, data_vars):
        grps = [f'{g}/ku' for g in list(netCDF4.Dataset(ncfile).groups) if 'data_20' in g]
        return DatasetReader(src=ncfile, data_vars=data_vars, file_level=FileLevel.L2, group=grps[0] if len(grps) else '')
