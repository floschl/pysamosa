...
class DatasetReader():
    def __init__(self, src, data_vars, group='', file_level=None):
        ...
        if src.is_dir():
            self.ncfiles = sorted(
                list(src.rglob('*.nc')), key=lambda f: str(f.name).replace('RES_', '')[16:31])
        else:
    ...
    def get_nc_filename(self, file_id=None):
        try:
            if file_id and isinstance(self.ncfiles, list):
                file_pattern = file_id_mappings[file_id] if not self.file_level else file_id_mappings[file_id].replace(
                    '1B', self.file_level.value.ljust(2, '_'))
                nc_filename = [
                    nc for nc in self.ncfiles if bool(
                        re.match(
                            f'(.*){file_pattern}(.*)',
                            str(nc)))][0]
            else:
