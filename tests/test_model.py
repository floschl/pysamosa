...
def test_waveform_single_look():
    models_sets = ModelSettings.get_default_sets(st=SensorType.S3, retracker_basetype=RetrackerBaseType.SAM)
    sm = SamosaModel(sensor_sets=SensorSettings(), wf_sets=WaveformSettings(), model_sets=models_sets)
    ...
    for s in swh:
        wf = sm.get_waveform_singlelook(Pu=P_u, Hs=s, t0_ns=t_0, fa_Hz=0.0, model_params=ModelParameter())
...
def test_multilooked_oversample():
    ...
    for oversampling_factor, m in zip([1, 4],['x', None]):
        wf_sets = WaveformSettings.get_default_src_type(L1bSourceType.EUM_S3, internal_oversampling_factor=oversampling_factor)
        model_sets = ModelSettings(retracker_basetype=RetrackerBaseType.SAM)
        sm = SamosaModel(model_sets=model_sets, wf_sets=wf_sets, sensor_sets=SensorSettings())
        ...
        wf = sm.get_waveform_multilook(Pu=Pu, Hs=swh, t0_ns=t_0_ns, nu=nu, model_params=model_params)

        # calculate interpolated x_vector to map all models on the same x-axis scale
        x_interp = np.append(np.arange(0, 127 + 1/oversampling_factor, 1/oversampling_factor), np.arange(127 + 1/oversampling_factor, 128, 1 / oversampling_factor))

        plt.plot(x_interp, wf, linewidth=0.5, marker=m, markersize=0.8, label=f'wf_model Np={128*oversampling_factor}')
...
def test_different_model_len():
    ...
    # for model_len, m in zip([512, 509],['x', None]):
        wf_sets = WaveformSettings.get_default_src_type(L1bSourceType.EUM_S3, zp_oversampling_factor=oversampling_factor)
        ...
        epoch_ref_gate = 64
        model_params = ModelParameter(epoch_ref_gate=epoch_ref_gate * oversampling_factor)
        wf = sm.get_waveform_multilook(Pu=Pu, Hs=swh, t0_ns=t_0_ns, nu=nu, model_params=model_params)
        ...
        plt.plot(x_gates, wf, linewidth=0.5, marker=m, markersize=1, label=f'wf_model_len={model_len}')
        # plt.axvline(epoch_ref_gate, label=f'epoch_ref_gate (of={oversampling_factor})')
...
def test_oversampled_model():
    ...
    sm_of1 = SamosaModel(model_sets=ModelSettings(retracker_basetype=RetrackerBaseType.SAM), wf_sets=WaveformSettings.get_default_src_type(L1bSourceType.EUM_S3, internal_oversampling_factor=1), sensor_sets=SensorSettings())
    model_params = ModelParameter(epoch_ref_gate=epoch_ref_gate)
    wf_of1 = sm_of1.get_waveform_multilook(Pu=Pu, Hs=swh, t0_ns=t_0_ns, nu=nu, model_params=model_params)

    wf_of1 = oversample_wf(wf_of1, internal_oversampling_factor)

    # oversampling=1 and oversample by internal_oversampling_Factor
    sm_of4 = SamosaModel(model_sets=ModelSettings(retracker_basetype=RetrackerBaseType.SAM), wf_sets=
    WaveformSettings.get_default_src_type(L1bSourceType.EUM_S3,
    ...
    model_params = ModelParameter(epoch_ref_gate=epoch_ref_gate)
    wf_of4 = sm_of4.get_waveform_multilook(Pu=Pu, Hs=swh, t0_ns=t_0_ns, nu=nu, model_params=model_params)
    ...
    plt.plot(wf_of1, linewidth=0.3, marker='x', markersize=0.1, label=f'model.internal_of={1} + oversample')
    plt.plot(wf_of4, linewidth=0.3, marker='o', markersize=0.3, label=f'model.internal_of={internal_oversampling_factor}')
    # plt.axvline(epoch_ref_gate, label=f'epoch_ref_gate (of={oversampling_factor})')
...
def test_multilooked_vary_Pu():
    ...
    model_params = ModelParameter(epoch_ref_gate=64)
    for p in Pu:
        wf = sm.get_waveform_multilook(Pu=p, Hs=swh, t0_ns=t_0, nu=nu, model_params=model_params)
        # wf = wf + np.random.randn(len(wf)) * 0.01 * np.nanmax(wf)  # NOISE ADDITION (not in the SAMOSA documentation )
