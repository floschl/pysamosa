...
class MonteCarloSimulator:
    ...
    def __call__(
        ...
        add_set_name = add_set_name if isinstance(
            add_set_name, list) else [add_set_name]
        ...
        for i in range(n_realisations):
            ...
            symbols = {"self": self}
            res_fit = {**res_fit,
                       **{col.split('.')[-1]: eval(f'self.{col}',
                                                   symbols) for col in add_set_name}}
...
def plot_cost_func_vs_swh(
    ...
    legend_kwargs = {
        ...
        **legend_kwargs} if legend_kwargs is not None else default_legend_kwargs

    default_label_kw = {'fontsize': 5}
    label_kw = {
        **default_label_kw,
        **label_kw} if label_kw is not None else default_label_kw

    default_ticks_kw = {'labelsize': 5}
    ticks_kw = {
        **default_ticks_kw,
        **ticks_kw} if ticks_kw is not None else default_ticks_kw

    subplot_name_kw = subplot_name_kw if subplot_name_kw is not None else {}

    func = cost_functions[cost_func]
    df_func_vs_swh_exp = df.groupby(
        by='swh_expected').apply(
        lambda g: func(
            g[var_type],
            g[f'{var_type}_expected']))

    _ax = df_func_vs_swh_exp.plot(
        marker='o', markersize=3, ax=_ax, **ax_kwargs)
    ...
    _ax.set_xlabel(
        f'expected {label_unit[var_type][0]} [{label_unit[var_type][1]}]',
        **label_kw)
    _ax.set_ylabel(
