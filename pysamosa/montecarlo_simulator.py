...
from pysamosa.common_types import ModelSettings, RetrackerBaseType, RetrackerSettings, FittingSettings, \
    WaveformSettings, SENSOR_SETS_DEFAULT_S3
...
class MonteCarloSimulator:
    ...
    def __init__(self, rbt : RetrackerBaseType,
        ...
        self.wf_sets_model = wf_sets_model if wf_sets_model is not None else wf_sets_retracker

        self.sr = retracker.SamosaRetracker(retracker_basetype=RetrackerBaseType.SAM,
    ...
    def __call__(self, swh, *, n_realisations, add_set_name=None, add_thermal_speckle_noise=True, add_interference=False):
        add_set_name = [] if add_set_name is None else add_set_name
        add_set_name = add_set_name if isinstance(add_set_name, list) else [add_set_name]

        l1b_sim = L1bSimulator(model_sets=self.model_sets, wf_sets=self.wf_sets_model, swh=swh, Pu=1.0, add_thermal_speckle_noise=add_thermal_speckle_noise, add_interference=add_interference)
        l1b_sim_it = iter(l1b_sim)

        logging.info(f'Running montecarlo simulation with SWH={l1b_sim.swh}, epoch={l1b_sim.epoch_ns}ns, Pu_norm={l1b_sim.Pu}')
        ...
        for i in range(n_realisations):
            ...
            res_fit = self.sr.fit_wf(l1b_data_single=l1b_data_single, model_params=model_params)
            ...
            symbols = {"self": self}  # hack because of NameError when eval tries to evaluate self
            res_fit = {**res_fit, **{col.split('.')[-1]: eval(f'self.{col}', symbols) for col in add_set_name}}
    ...
    def multi_proc(self, swh, max_workers=None, **kwargs):
        ...
        if max_workers == 1:
            ...
            with futures.ProcessPoolExecutor(max_workers=max_workers) as pool:
                # treat kwargs as fixed arguments for monte_sim.__call__ and swh as variable param on each process to parallelise
                fit_results_list = pool.map(partial(montesim_call_wrapper, self, kwargs), swh, chunksize=5)

        df = pd.concat(fit_results_list, ignore_index=True)

        return df

def plot_cost_func_vs_swh(df, *, var_type, cost_func : CostFunctionType, ax=None, ax_kwargs=None, legend_kwargs=None, label_kw=None, ticks_kw=None, subplot_name=None, subplot_name_kw=None):
    _ax = ax if ax is not None else plt.gca()
    ax_kwargs = ax_kwargs if ax_kwargs is not None else {}
    default_legend_kwargs = {'fontsize': 5}
    legend_kwargs = {**default_legend_kwargs, **legend_kwargs} if legend_kwargs is not None else default_legend_kwargs

    default_label_kw = {'fontsize': 5}
    label_kw = {**default_label_kw, **label_kw} if label_kw is not None else default_label_kw

    default_ticks_kw = {'labelsize': 5}
    ticks_kw = {**default_ticks_kw, **ticks_kw} if ticks_kw is not None else default_ticks_kw

    subplot_name_kw = subplot_name_kw if subplot_name_kw is not None else {}

    func = cost_functions[cost_func]
    df_func_vs_swh_exp = df.groupby(by='swh_expected').apply(lambda g : func(g[var_type], g[f'{var_type}_expected']))
    ...
    _ax.set_xlabel(f'expected {label_unit[var_type][0]} [{label_unit[var_type][1]}]', **label_kw)
    _ax.set_ylabel(f'{label_unit[var_type][0]} {cost_func.value.lower().replace("_", " ")} [{label_unit[var_type][1]}]', **label_kw)
