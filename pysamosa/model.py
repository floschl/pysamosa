...
from pysamosa.common_types import ModelSettings, ModelParameter, RetrackerBaseType, WaveformSettings, SensorSettings, SensorType
...
def get_region_argmax(wf, region_center, n_before_after):
    return np.argmax(wf[region_center - n_before_after:region_center + n_before_after]) + (region_center - n_before_after)
...
class SamosaModel():
    ...
    def __init__(self, model_sets : ModelSettings,
    ...
    def _check_param(self, Pu: float, Hs: float, t0_ns: float):
        if not self.model_sets.Disable_LUT_Flag and not (self.luts_samosa['lut_alpha_p'][0][0] <= Hs <= self.luts_samosa['lut_alpha_p'][-1][0]):
            raise RuntimeError(f"Hs must be within range [{self.luts_samosa['lut_alpha_p'][0][0]}, {self.luts_samosa['lut_alpha_p'][-1][0]}]")

    def get_waveform_singlelook(self, Pu: float, Hs: float, t0_ns: float, fa_Hz: float, nu: float = 0.0, *, model_params, custom_gamma0=None):
        '''
        ...
        # Re = np.sqrt(CONST_A**2 * np.cos(lat_geocentric)**2 + CONST_B**2 * np.sin(lat_geocentric)**2)  #more accurate (lat-dependent) calculus of Earth' radius
        Re = np.sqrt(CONST_A**2 * np.cos(model_params.lat_rad)**2 + CONST_B**2 * np.sin(model_params.lat_rad)**2)
        alpha = 1 + h / Re

        # SL3. Calculate model_paramsmodel_params.lat_radlat_rad new parameters
        prf_hz = self.sensor_sets.prf_Hz if self.sensor_sets.prf_Hz is not None else 1 / model_params.pri_hz
        ...
        alphax = self.sensor_sets.sh_x * 8 * np.log(2) / (h ** 2 * self.sensor_sets.theta_x_rad ** 2)
        alphay = self.sensor_sets.sh_y * 8 * np.log(2) / (h ** 2 * self.sensor_sets.theta_y_rad ** 2)
        Tb = self.sensor_sets.n_b / prf_hz
        Lx = (CONST_C * h) / (2 * model_params.Vs_m_per_s * self.sensor_sets.f_c_Hz * Tb)
        ...
        if self.model_sets.retracker_basetype == RetrackerBaseType.SAMPLUS and 0.0 <= model_params.dist2coast < 10.0:
            alpha_p = 0.55
        elif self.model_sets.Disable_LUT_Flag:
            alpha_p = self.model_sets.alpha_p_mean
        else:
            alpha_p = self.luts_samosa['lut_alpha_p'][bisect_left(self.luts_samosa['lut_alpha_p'][:, 0], Hs), 1]

        # SL5. Calculate GL and GLK
        track_sign = -1 if model_params.ascending else 1
        orbit_slope = track_sign * ((CONST_A**2 - CONST_B**2) / (2 * Re**2)) * np.sin(2 * model_params.lat_rad) - (-model_params.h_rate_m_per_s / model_params.Vs_m_per_s)
        Ls = orbit_slope * h / (alpha * Lx)
        if self.model_sets.Enable_Slope_Effect_Flag:  #for S6: Enable_Slope_Effect_Flag is inherently turned on because a different GL formula is used
        ...
        # theta_l = Lx / h * (L - Ls)
        # GL = 1 / (self.B_r_Hz_oversampled * np.sqrt(sigma_p**2 * (1 + (Lx / Lz)**2 * (alpha * theta_l)**2) + (2 * sigmaz / CONST_C)**2))  #alternative formula from Dinardo et al. 2018
        GL = 1 / np.sqrt(alpha_p**2 + alpha_p**2 * gamma**2 + sign * (Hs / (4 * Lz))**2)
        ...
        if self.model_sets.retracker_basetype == RetrackerBaseType.SAMPLUSPLUS and custom_gamma0 is not None:
            gamma0 = custom_gamma0
        else:
            gamma0 = np.exp(-alphay * yp**2
                            -alphax * (XL - xp)**2
                            -XL**2 * nu / h**2
                            -(alphay + nu/h**2) * YK**2)\
                     * np.cosh(2 * alphay * yp * YK)
        Const = self.model_sets.alpha_p_mean**2 * np.sqrt(2 * np.pi)  # here it is intended to use the constant alpha_p term

        # SL6. Calculate Tk
        Tk = np.zeros(Np)
        Tk[K>0] = (1 + nu / (alphay * h**2)) - (yp / (Ly * np.sqrt(K[K>0]))) * np.tanh(2 * alphay * yp * Ly * np.sqrt(K[K>0]))
        ...
        if self.model_sets.Disable_SAM_F1_Flag:
            ...
            Pr_SL = Const * np.sqrt(GL) * gamma0 * (f_0_l_k + (sigmaz / Lg) * (sigmaz / Lz) * GL * Tk * f_1_l_k)
    ...
    def get_waveform_multilook(self, Pu, Hs, t0_ns, nu=0, *, model_params=None):
        ...
        if self.model_sets.Ideal_Beam_Ang_Stack_flag or not len(model_params.beam_ang_stack_rad):
            ...
            beam_ang_stack_full = model_params.beam_ang_stack_rad[~np.isnan(model_params.beam_ang_stack_rad)]

            # reduce beam_ang_stack_full
            n_looks_to_sum = self.model_sets.n_effective_looks
            # n_looks_to_sum = 0
            if n_looks_to_sum == 0:
                self.Beam_Ang_Stack_TS = model_params.beam_ang_stack_rad[~np.isnan(model_params.beam_ang_stack_rad)]
            else:
                self.Beam_Ang_Stack_TS = beam_ang_stack_full[beam_ang_stack_full.shape[0] // 2 - n_looks_to_sum // 2:beam_ang_stack_full.shape[0] // 2 + n_looks_to_sum // 2]
        ...
        factor = 1
        Beam_Index = np.unique((np.round(Beam_Indices_float / factor)).astype(int)) * factor
        ...
        if self.model_sets.retracker_basetype == RetrackerBaseType.SAMPLUSPLUS:
            ripa = RipAnalyser(model_params.rip, sensor_sets=self.sensor_sets, model_params=model_params)
        ...
        for j,fa in enumerate(Dopp_Freq_Stack_TS):
            ...
            if custom_gamma0 is not None:
                self.Pr_Stack[j] = self.get_waveform_singlelook(Pu=Pu, Hs=Hs, t0_ns=t0_ns, fa_Hz=fa, nu=nu, model_params=model_params, custom_gamma0=custom_gamma0[:,j])
            else:
                self.Pr_Stack[j] = self.get_waveform_singlelook(Pu=Pu, Hs=Hs, t0_ns=t0_ns, fa_Hz=fa, nu=nu, model_params=model_params)
        ...
        if model_params.stack_mask_start_stop is not None:
            inds_stack_mask_mapped = get_closest_idxs(np.sort(Beam_Indices_float), np.sort(Beam_Index))
            stack_mask_start_stop_mapped = model_params.stack_mask_start_stop[inds_stack_mask_mapped].astype(int)
        ...
        if self.wf_sets.internal_oversampling_factor > 1:
            self.Pr_ML = np.append(self.Pr_ML, np.repeat(self.Pr_ML[-1], (self.wf_sets.internal_oversampling_factor - 1)))

        return self.Pr_ML

    def get_waveform_interference_mask(self, *, wf, t0_ns, model_params, fg_epoch_gates, n_normalise_grow, n_mask_grow, swh_max=None, mask_before_le=False):
        ...
        """
        if n_normalise_grow:
            wf = wf / get_region_max(wf, region_center=fg_epoch_gates, n_before_after=n_normalise_grow)
        else:
            wf = wf / np.max(wf)

        mask_max = self.get_waveform_singlelook(Pu=1.0, Hs=swh_max, t0_ns=t0_ns, nu=0.00, model_params=model_params, fa_Hz=0)
        mask_max = np.append(mask_max, np.repeat(mask_max[-1], self.wf_sets.internal_oversampling_factor - 1))
        ...
        # get indices of gates, for which the waveform is greater than the modelled envelope
        inds_bad = (wf.squeeze() > interference_ref_waveform.squeeze()).nonzero()[0].tolist()

        # grow region of each bad ind by n_grow
        if len(inds_bad):
            if n_mask_grow:
                inds_bad = [[i for i in range(j - n_mask_grow, j + n_mask_grow + 1)] for j in inds_bad]
                inds_bad = sorted(list(set(list(chain.from_iterable(inds_bad)))))  # flatten list and get unique set of bad inds
            else:
                inds_bad = list(set(inds_bad))  # flatten list and get unique set of bad inds

        # remove bad inds that are before fg_epoch_gates
        # inds_bad = [i for i in inds_bad if i >= fg_epoch_gates]

        # remove bad inds that are too close to the fg_epoch, meaning fg_epoch +- n_mask_grow
        inds_bad = [i for i in inds_bad if (np.abs(fg_epoch_gates - i) > n_mask_grow) ]
        inds_bad = [i for i in inds_bad if (np.abs(fg_epoch_gates - i) > n_normalise_grow) ]
