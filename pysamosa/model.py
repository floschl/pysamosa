...
def get_region_max(wf, region_center, n_before_after):
    return np.max(wf[region_center -
                     n_before_after:region_center +
                     n_before_after])


def get_region_argmax(wf, region_center, n_before_after):
    return np.argmax(wf[region_center - n_before_after:region_center +
                     n_before_after]) + (region_center - n_before_after)
...
def get_closest_idxs(array, values):
    ...
    prev_idx_is_less = ((idxs == len(array)) | (np.fabs(values - array[np.maximum(
        idxs - 1, 0)]) < np.fabs(values - array[np.minimum(idxs, len(array) - 1)])))
...
class SamosaModel():
    ...
    def __init__(self, model_sets: ModelSettings,
        ...
        self.luts_samosa = load_samosa_luts()[
            self.sensor_sets.sensor_type.value]

    def _check_param(self, Pu: float, Hs: float, t0_ns: float):
        if not self.model_sets.Disable_LUT_Flag and not (
                self.luts_samosa['lut_alpha_p'][0][0] <= Hs <= self.luts_samosa['lut_alpha_p'][-1][0]):
            raise RuntimeError(
    ...
    def get_waveform_singlelook(
        ...
        Np = int(self.wf_sets.np) - \
            (self.wf_sets.internal_oversampling_factor - 1)
        ...
        prf_hz = self.sensor_sets.prf_Hz if self.sensor_sets.prf_Hz is not None else 1 / \
            model_params.pri_hz
        ...
        alphax = self.sensor_sets.sh_x * 8 * \
            np.log(2) / (h ** 2 * self.sensor_sets.theta_x_rad ** 2)
        alphay = self.sensor_sets.sh_y * 8 * \
            np.log(2) / (h ** 2 * self.sensor_sets.theta_y_rad ** 2)
        Tb = self.sensor_sets.n_b / prf_hz
        Lx = (CONST_C * h) / (2 * model_params.Vs_m_per_s *
                              self.sensor_sets.f_c_Hz * Tb)
        ...
        if self.model_sets.retracker_basetype == RetrackerBaseType.SAMPLUS and 0.0 <= model_params.dist2coast < 10.0:
            ...
            alpha_p = self.luts_samosa['lut_alpha_p'][bisect_left(
                self.luts_samosa['lut_alpha_p'][:, 0], Hs), 1]

        # SL5. Calculate GL and GLK
        track_sign = -1 if model_params.ascending else 1
        orbit_slope = track_sign * ((CONST_A**2 - CONST_B**2) / (2 * Re**2)) * np.sin(
            2 * model_params.lat_rad) - (-model_params.h_rate_m_per_s / model_params.Vs_m_per_s)
        ...
        GL = 1 / np.sqrt(alpha_p**2 + alpha_p**2 * gamma **
                         2 + sign * (Hs / (4 * Lz))**2)
        ...
        # SL6. Calculate Tk
        Tk = np.zeros(Np)
        Tk[K > 0] = (1 + nu / (alphay * h**2)) - (yp / (Ly * np.sqrt(K[K > 0]))
        ...
        if self.model_sets.Disable_SAM_F1_Flag:
            ...
            Pr_SL = Const * np.sqrt(GL) * gamma0 * (f_0_l_k + \
                                    (sigmaz / Lg) * (sigmaz / Lz) * GL * Tk * f_1_l_k)

        return Pr_SL

    def get_waveform_multilook(
            self,
            Pu,
            Hs,
            t0_ns,
        ...
        # Improved sea level determination in the Arctic regions through development
        # of tolerant altimetry retracking, Eq. 5.2, pp. 47)
        lat_geocentric = np.arctan(
            (1 - CONST_F)**2 * np.tan(model_params.lat_rad))
        ...
        if self.model_sets.Ideal_Beam_Ang_Stack_flag or not len(
            ...
            beam_ang_stack_full = model_params.beam_ang_stack_rad[~np.isnan(
                model_params.beam_ang_stack_rad)]
            ...
            # n_looks_to_sum = 0
            if n_looks_to_sum == 0:
                self.Beam_Ang_Stack_TS = model_params.beam_ang_stack_rad[~np.isnan(
                    model_params.beam_ang_stack_rad)]
            else:
                self.Beam_Ang_Stack_TS = beam_ang_stack_full[beam_ang_stack_full.shape[0] //
                                                             2 - n_looks_to_sum // 2:beam_ang_stack_full.shape[0] // 2 + n_looks_to_sum // 2]
        ...
        Dopp_Freq_Stack_TS = (2 * Vs / lambda0) * \
            np.cos(self.Beam_Ang_Stack_TS)

        # ML4. Sub-setting the Stack
        dfa = prf_hz / self.sensor_sets.n_b
        Beam_Indices_float = Dopp_Freq_Stack_TS / dfa
        factor = 1
        Beam_Index = np.unique(
            (np.round(Beam_Indices_float / factor)).astype(int)) * factor
        ...
        for j, fa in enumerate(Dopp_Freq_Stack_TS):
            ...
            if custom_gamma0 is not None:
                self.Pr_Stack[j] = self.get_waveform_singlelook(
                    Pu=Pu, Hs=Hs, t0_ns=t0_ns, fa_Hz=fa, nu=nu, model_params=model_params, custom_gamma0=custom_gamma0[:, j])
            else:
                self.Pr_Stack[j] = self.get_waveform_singlelook(
                    Pu=Pu, Hs=Hs, t0_ns=t0_ns, fa_Hz=fa, nu=nu, model_params=model_params)
        ...
        if model_params.stack_mask_start_stop is not None:
            ...
            stack_mask_start_stop_mapped = model_params.stack_mask_start_stop[inds_stack_mask_mapped].astype(
                int)
        ...
        if self.wf_sets.internal_oversampling_factor > 1:
            self.Pr_ML = np.append(self.Pr_ML, np.repeat(
    ...
    def get_waveform_interference_mask(
        ...
        """
        if n_normalise_grow:
            wf = wf / get_region_max(wf,
                                     region_center=fg_epoch_gates,
                                     n_before_after=n_normalise_grow)
        else:
            wf = wf / np.max(wf)
        ...
        mask_max = self.get_waveform_singlelook(
            ...
            model_params=model_params,
            fa_Hz=0)
        mask_max = np.append(mask_max, np.repeat(
            mask_max[-1], self.wf_sets.internal_oversampling_factor - 1))
        mask_max = mask_max / np.max(mask_max)
        interference_ref_waveform = mask_max

        mask_close_to_zero = np.isclose(
            interference_ref_waveform, 0.0, atol=1e-2)
        ...
        # modelled envelope
        inds_bad = (wf.squeeze() > interference_ref_waveform.squeeze()).nonzero()[
            0].tolist()
        ...
        if len(inds_bad):
            if n_mask_grow:
                inds_bad = [
                    [i for i in range(j - n_mask_grow, j + n_mask_grow + 1)] for j in inds_bad]
                # flatten list and get unique set of bad inds
                inds_bad = sorted(
                    list(set(list(chain.from_iterable(inds_bad)))))
            else:
                # flatten list and get unique set of bad inds
