...
if __name__ == "__main__":
    nc_src_base_path = Path('/nfs/DGFI145/A/original_data/sentinel6a/JASON_CS_S6A_L1B_ALT_HR_NTC_F_V2/')
    ...
    # select files
    l1b_files = [f for f in sorted(nc_src_base_path.rglob('*.nc')) if bool(re.search(cycle_pass_pattern, str(f)))]

    l1b_src_type = L1bSourceType.EUM_S6_F04 if 'f04' in str(l1b_files[0]).lower() else L1bSourceType.EUM_S6_F06
    ...
    df_l1b_files = pd.DataFrame({'file': l1b_files, 'cycle': cycles, 'ppass': passes, 'start_date': start_dates, 'end_date': end_dates})
    ...
    rp = RetrackerProcessor(l1b_source=df_l1b_files.file.values, l1b_data_vars=data_vars_s6['l1b'],
                            rp_sets=rp_sets,
