...
class SensorSettings(BaseModel):
    ...
    theta_x_rad: float = np.radians(1.338)  # 3dB beamwidth along track [rad], from S.Dinardo (mail 29.05.20)
    theta_y_rad: float = np.radians(1.338)  # 3dB beamwidth across track [rad], from S.Dinardo (mail 29.05.20)
    prf_Hz: float = 1 / (4488/80e6) # Pulse repetition frequency [Hz] (Determined by reverse engineering, confirmed by W. Smith), from S.Dinardo (mail 02.06.20), 4488 = pulse repetition interval

    # tau_u: float = 44.8e-6  # useful pulse length [s]
    n_b: int = 64  # number of pulses per Burst [-]
    bri: float = 1018710 * 1/80e6  # Burst Repetition Interval [s] (Determined by reverse engineering, confirmed by W. Smith)
...
class WaveformSettings(BaseModel):
    ...
    def get_default_src_type(st : L1bSourceType, **kwargs):
        if st is L1bSourceType.GPOD:
            return WaveformSettings(**{**{'zp_oversampling_factor': 2, 'np': 256, **kwargs}})
        elif st is L1bSourceType.EUM_S3:
            return WaveformSettings(**{**{'zp_oversampling_factor': 1, 'np': 128, **kwargs}})
        elif st is L1bSourceType.EUM_CS:
            return WaveformSettings(**{**{'zp_oversampling_factor': 2, 'np': 128, **kwargs}})
    ...
    def set_default_np(cls, v, *, values, **kwargs):
        ...
        return np * values['zp_oversampling_factor'] * values['internal_oversampling_factor']
...
class RetrackerSettings(BaseModel):
    retracker_basetype: RetrackerBaseType
    Disable_ML_Flag: bool = False  #flag to disable the usage of multilook (0 -> Enable ML; 1 -> Disable Multilook)
    Wf_Norm_Flag: bool = True  # Flag to control activation of waveform normalization (1 = true ; 0 = false);
    ...
    # Wf_Norm_Last: int = None  # Last gate of the waveform to search for the max. amplitude
    ...
    Wf_Fit_First_Bin: int = -1  # -1 means fit from beginning of waveform, without zero-padding factor
    Wf_Fit_Last_Bin: int = -1  # -1 means fit until end of waveform, without zero-padding factor
    # Wf_Fit_Activate_Flag = 1 #Flag to control the activation of the waveform fitting (1 = true ; 0 = false);
    Thr: float = 3.0  # Threshold parameter greater than 1 used to test Wf_max against TN
    # TN_Flag = 0 #flag to determine how to deal with Thermal Noise (TN_Flag=1 -> retracked ; TN_Flag=0 constant/estimated)

    second_retracking_step_samplus : bool = False
    n_effective_looks : int = 0
    normalise_wf_by_fg_region : int = 0  # 0:disabled, >0:number of samples around dynamic_fg_epoch
    leading_edge_weight_factor : float = 1.0  # increases the weights for the detected leading edge
    ...
    auto_detect_s6_rmc : bool = True  # auto-detects RAW/RMC mode for S6 and adjusts Wf_Fit_First_Bin/Wf_Fit_Last_Bin

    subwaveform_mode : bool = False
    subwaveform_n_gates_after_le : int = 5

    fit_zero_doppler: bool = False  # fits the zero-doppler beam (reasonable for FF-SAR-processed waveforms)
...
class ModelParameter(BaseModel):
    ...
    def warning_defaults(cls, values):
        ...
        if fields_not_set:
            logging.debug('WARNING: the following params were not set, now taking defaults: {}'.format(','.join(fields_not_set)))
        return values
...
class ModelSettings(BaseModel):
    retracker_basetype: RetrackerBaseType
    Disable_SAM_F1_Flag: bool = False  #flag to disable the computation of first order term in the SAMOSA model (0 -> Enable First Order term; 1 -> Disable First order term)
    Disable_LUT_Flag: bool = False  #flag to disable the computation of alph_P by LUT (0 -> Use LUT to extract the alph_P value; 1 -> use a constant value for alph_P)
    Ideal_First_Look_Index_ML: int = -106  # Index for the first idealized beam accumulated in the multilooking stage
    Ideal_Last_Look_Index_ML: int = 106  # Index for the last idealized beam accumulated in the multilooking stage
    Ideal_Beam_Ang_Stack_flag: bool = False  # 1 -> idealized beam angles are computed from simple geometric considerations; 0 -> Use the values from data.
    Enable_Slope_Effect_Flag: bool = False  #flag to enable the slope effect compensation (1 -> Enable Slope Effect Compensation; 0 -> Disable Slope Effect Compensation)
    alpha_p_mean: float = 0.5  # as in Dinardo2020, not as in DPM 2.5.2: 1 / (0.886 * np.sqrt(2 * np.pi))  # ~0.45, PTR Gaussian approximation coefficient value
    n_effective_looks: int = 0
    fit_zero_doppler: bool = False  # fits the zero-doppler beam (reasonable for FF-SAR-processed waveforms)
...
class FittingSettings(BaseModel):
    retracker_basetype: RetrackerBaseType
    Fit_Var_2_Init_Hs: float = 2.0 # Initial guess for the 2nd fitted variable (Hs)
    Fit_Var_3_Init_Pu: float = 1.0  # Initial guess for the 3rd fitted variable (Pu)
    Fit_Var_4_Init_TN: float = 0.0  # Initial guess for the 4th fitted variable (ThNoise). Activge if TN_Flag=1.
    Fit_Var_5_Init_nu: float = 0  # Initial guess for the optional fitted variable (nu).
    Fit_Var_1_MinMax_epoch: tuple = (-600.0, 600.0)  # Minimum/maximum allowed value for the 1st fitted variable (t0) in nanoseconds
    Fit_Var_2_MinMax_Hs: tuple = (-0.5, 20)  # Minimum/maximum allowed value for the 2nd fitted variable (Hs)
    Fit_Var_3_MinMax_Pu: tuple = (0.2, 1.5)  # Minimum/maximum allowed value for the 3rd fitted variable (Pu)
    Fit_Var_4_MinMax_TN: tuple = (0.0, 0.05)  # Minimum/maximum allowed value for the 4th fitted variable (TN).
    Fit_Var_5_MinMax_nu: tuple = (0, 1e9)  # Minimum/maximum allowed value for the optional fitted variable (nu)
    ...
    trf_threshold: float = 1e-5  # Threshold for xtol, ftol, and gtol tolerances of TRF fitting algorithm
    trf_stepsize: float = 1e-2  # Threshold for xtol, ftol, and gtol tolerances of TRF fitting algorithm
    limit_epoch_around_fg_n : int = 0  # sets strict epoch boundaries in gates around fg epoch (0: disabled)
