...
def detect_leading_edge(wf, fg_epoch=None, normalise_wf_by_fg_region=None, append_first_falling_edge=False):
    ...
    if fg_epoch and normalise_wf_by_fg_region:
        wf = wf / get_region_max(wf=wf, region_center=fg_epoch, n_before_after=normalise_wf_by_fg_region)
    else:
        wf = wf / np.max(wf)

    fg_epoch = np.argmax(wf) if fg_epoch is None else fg_epoch

    # get the numerical gradient of the waveform and all rising edges above a certain threshold
    ...
    rising_edges_shortened = [list(r) for r in rising_edges if (fg_epoch - 2 * normalise_wf_by_fg_region) <= r[-1] <= (fg_epoch + normalise_wf_by_fg_region)]
    rising_edges_shortened = list(filter(None, rising_edges_shortened))  # remove empty list

    # selected_le_gates = sorted(rising_edges_left_fg_shortend, key=len, reverse=True)[0]  # sort rising edges by width
    selected_le_gates = sorted(rising_edges_shortened, key=lambda e: wf[e[-1]] - wf[e[0]], reverse=True)  # sort rising edges by height

    # fill potential gaps in series
    if len(selected_le_gates) and len(selected_le_gates[0]):
        selected_le_gates = np.arange(selected_le_gates[0][0], selected_le_gates[0][-1] + 1)
    else:
    ...
    if append_first_falling_edge:
        ...
        falling_edge_after_le = [fe for fe in falling_edges if (fe[0] - 1) == selected_le_gates[-1]][0]
