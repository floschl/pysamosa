...
from pysamosa.data_access import _read_dataset_vars_from_ds, data_vars_s3, get_subset_dataset, get_model_param_obj_from_l1b_data
from pysamosa.retracker import SamosaRetracker
from pysamosa.conf_params import CONST_C


def get_tracker_shift_n_samps(tracker_range_m: np.ndarray, alt_m: np.ndarray, t0_sampling_gate_ns: float) -> np.ndarray:
    """
...
def shift_waveform_by_n(wfs: np.ndarray, shifts: np.ndarray):
    ...
    if wfs.shape[0] != len(shifts):
        raise RuntimeError('shifts vector must be of the same length as the number of waveforms. ')
...
def get_inds_sliding_window(n_total_len, n):
    ...
    # pre- and append for n-1 times the same window to account for sliding in/out effect
    if n_total_len > n:
        n_prepend = int(n // 2)
        n_append = n - (n_prepend + 1)
        windows_no_slidein = list(sliding_window(np.arange(n_total_len), n))
        windows = [*[(w, i) for i,w in enumerate([windows_no_slidein[0][0]] * n_prepend)], *windows_no_slidein, *[(w,(n // 2 + 1 + i)) for i,w in enumerate([windows_no_slidein[-1][0]] * n_append)]]
    else:
        windows = [(win,i) for i,win in enumerate([list(range(n_total_len))] * n_total_len)]
...
def get_single_dynamic_first_guess_epochs(wfs : np.ndarray, tracker_range: np.ndarray, alt_m: np.ndarray, bu_bw_Hz, shifted_window_abs_inds_center : list):
    n_gates = wfs.shape[1]

    # shifted_window = shifted_window_abs_inds_center[0]
    rel_ind_window_center = shifted_window_abs_inds_center[1]

    # get shift in n samples that aligns all waveforms in terms of its absolute ranges
    n_shifts = get_tracker_shift_n_samps(tracker_range_m=tracker_range, alt_m=alt_m, t0_sampling_gate_ns=1 / bu_bw_Hz)

    # if there is a very abrupt jump in the tracker range, then limit the adjacent samples to have a maximum shift of half of the wf length
    mask_possible_shifts = np.abs((n_shifts - n_shifts[rel_ind_window_center])) < (n_gates // 3)

    # adjust possible indices
    rel_ind_window_center_possible = rel_ind_window_center - np.sum(~mask_possible_shifts[0:rel_ind_window_center])  # accounts for invalid tracker range jumps
    ...
    wfs_range_aligned_norm = np.apply_along_axis(lambda wf: wf / np.max(wf), 1, wfs_range_aligned)

    if all(np.apply_along_axis(lambda wf: np.allclose(wf[~np.isnan(wf)], 0.0), axis=1, arr=wfs_range_aligned_norm)):
        return 0, np.full((n_gates,), np.nan), np.full((n_gates,), np.nan)

    # if there are wfs with all-zero or all-nan, set them to 1 -> no effect on pointwise product
    wfs_range_aligned_norm = np.apply_along_axis(lambda wf: np.ones(len(wf)) if all(np.isnan(wf)) or np.isclose(np.mean(wf), 0.0, atol=1e-25) else wf, 1, wfs_range_aligned_norm)
...
def get_dynamic_first_guess_epochs(wfs: np.ndarray, tracker_range: np.ndarray, alt_m: np.ndarray, bu_bw_Hz, fg_epoch_adjacent_meas = 20, n_procs=1) -> np.ndarray:
    ...
    if n_procs == 1:
        for abs_ind, shifted_window_abs_inds_center in enumerate(get_inds_sliding_window(wfs.shape[0], n=fg_epoch_adjacent_meas)):
            ...
            new_fg_epoch, _, _ = get_single_dynamic_first_guess_epochs(wfs=wfs_window, tracker_range=tracker_range_window, alt_m=alt_m_window, bu_bw_Hz=bu_bw_Hz, shifted_window_abs_inds_center=shifted_window_abs_inds_center)
        ...
        with futures.ProcessPoolExecutor(max_workers=n_procs) as pool:
            shifted_window_abs_inds_center = get_inds_sliding_window(wfs.shape[0], n=fg_epoch_adjacent_meas)
            ...
            tracker_range_gen = gen_subset_view(tracker_range, shifted_window_abs_inds_center)

            for abs_ind, (new_fg_epoch, _, _) in enumerate(pool.map(get_single_dynamic_first_guess_epochs, wfs_gen, tracker_range_gen, alt_m_gen, repeat(bu_bw_Hz), shifted_window_abs_inds_center, chunksize=100)):
                if 0 < new_fg_epoch < n_gates:
                    new_fg_epochs[abs_ind] = new_fg_epoch
                else:
                    new_fg_epochs[abs_ind] = np.argmax(wfs[abs_ind])
...
def retrack_single_meas(*, sr, rp_sets, n_offset, nc_l1bs, datavars, sensor_sets):
    ...
    if rp_sets.do_dynamic_fg_epoch:
        # read n samples before the selected ones, required for estimation dynamic first guess epoch
        n_inds_total = rp_sets.dynamic_fg_epoch_n_adjacent_meas
        n_pre = n_inds_total // 2

        l1b_data =  _read_dataset_vars_from_ds(nc_filename=nc_l1bs, data_var_names=datavars, n_offset=n_offset - n_pre, n_inds=n_inds_total)
        ...
        l1b_data = _read_dataset_vars_from_ds(nc_filename=nc_l1bs, data_var_names=datavars, n_offset=n_offset, n_inds=1)
    ...
    fg_epoch = l1b_data_single['dynamic_fg_epoch'] if 'dynamic_fg_epoch' in l1b_data_single else np.argmax(l1b_data_single['wf'])
    ...
    if sr.retrack_sets and sr.retrack_sets.normalise_wf_by_fg_region:
        wf_meas = res_fit['wf'] / SamosaRetracker.get_wf_max(res_fit['wf'], retrack_sets=sr.retrack_sets, fg_epoch=fg_epoch)
    else:
