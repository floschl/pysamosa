...
def get_tracker_shift_n_samps(
        tracker_range_m: np.ndarray,
        alt_m: np.ndarray,
        t0_sampling_gate_ns: float) -> np.ndarray:
    """
...
def get_inds_sliding_window(n_total_len, n):
    ...
    if n_total_len > n:
        ...
        windows = [*[(w,
                      i) for i,
                     w in enumerate([windows_no_slidein[0][0]] * n_prepend)],
                   *windows_no_slidein,
                   *[(w,
                      (n // 2 + 1 + i)) for i,
                     w in enumerate([windows_no_slidein[-1][0]] * n_append)]]
    else:
        windows = [(win, i) for i, win in enumerate(
            [list(range(n_total_len))] * n_total_len)]
...
def get_single_dynamic_first_guess_epochs(
    ...
    n_shifts = get_tracker_shift_n_samps(
        ...
        alt_m=alt_m,
        t0_sampling_gate_ns=1 / bu_bw_Hz)
    ...
    mask_possible_shifts = np.abs(
        (n_shifts -
         n_shifts[rel_ind_window_center])) < (
        n_gates //
        3)

    # adjust possible indices
    rel_ind_window_center_possible = rel_ind_window_center - \
        np.sum(~mask_possible_shifts[0:rel_ind_window_center])  # accounts for invalid tracker range jumps
    n_shifts = n_shifts[mask_possible_shifts]
    n_shifts -= int(np.mean(n_shifts))  # demean

    wfs_range_aligned = shift_waveform_by_n(
        wfs[mask_possible_shifts, :], -n_shifts)
    ...
    if all(np.apply_along_axis(lambda wf: np.allclose(
            wf[~np.isnan(wf)], 0.0), axis=1, arr=wfs_range_aligned_norm)):
    ...
    new_fg_epoch = ind_max_adjacent_wfs + \
        n_shifts[rel_ind_window_center_possible]
    ...
    if n_procs == 1:
        for abs_ind, shifted_window_abs_inds_center in enumerate(
            ...
            new_fg_epoch, _, _ = get_single_dynamic_first_guess_epochs(
                wfs=wfs_window, tracker_range=tracker_range_window, alt_m=alt_m_window, bu_bw_Hz=bu_bw_Hz, shifted_window_abs_inds_center=shifted_window_abs_inds_center)
        ...
        with futures.ProcessPoolExecutor(max_workers=n_procs) as pool:
            ...
            for abs_ind, (new_fg_epoch, _, _) in enumerate(pool.map(get_single_dynamic_first_guess_epochs, wfs_gen,
                                                                    tracker_range_gen, alt_m_gen, repeat(bu_bw_Hz), shifted_window_abs_inds_center, chunksize=100)):
                if 0 < new_fg_epoch < n_gates:
                    new_fg_epochs[abs_ind] = new_fg_epoch
                else:
                    new_fg_epochs[abs_ind] = np.argmax(wfs[abs_ind])
...
def retrack_single_meas(
    *,
    ...
    fg_epoch = l1b_data_single['dynamic_fg_epoch'] if 'dynamic_fg_epoch' in l1b_data_single else np.argmax(
        l1b_data_single['wf'])
