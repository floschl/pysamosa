...
def convert_eum_mqe_to_misfit(mqe): return np.sqrt(100 * np.sqrt(mqe))
...
def _read_dataset_vars_from_ds(
    *,
    ...
    last_ind = get_last_ind(n_offset=n_offset,
                            n_inds=n_inds,
    ...
    def get_data_var(_v):
        ...
        # general conversions
        if 'lat' in _v.lower() or 'lon' in _v.lower(
        ) or 'off_nadir' in _v.lower() or 'beam' in _v.lower():
            is_deg_unit = hasattr(
                ds[_v], 'units') and 'deg' in str(
                ds[_v].units).lower()
            # is_rad_unit = (hasattr(ds[_v], 'units') and 'rad' in ds[_v].units)
            if is_deg_unit or (not is_deg_unit and any(
                    (abs(ds[_v]) > np.pi / 2).values.ravel())):
                var = np.radians(var)
    ...
    for k, v in data_var_names.items():
        if v in dir(ds) or 'const' in v:
            is_eumetsat = (('contact' in dir(ds) and 'eumetsat' in ds.contact) or 'EUM' in str(
                nc_filename)) and 'processor' not in dir(ds)
            is_gpod = 'NetCDF_CreatedBy' in dir(
                ds) and 'SARvatore' in ds.NetCDF_CreatedBy
            ...
            if is_gpod and 'oversampling_factor' in k:
                ...
                wf_oversampling_factor = float(
                    ds.WAVEFORM_OVERSAMPLING_FACTOR) if hasattr(
                    ds, 'WAVEFORM_OVERSAMPLING_FACTOR') else 1
                ...
                data_var = np.datetime64(
                    ds.SENSING_START_UTC_TIME[:-1]) + get_data_var(v)
            elif is_gpod and 'time' in v and is_cs:
                dt_str = ds.SENSING_START[1:]
                dt_str = dt_str.replace(
                    dt_str[3:6], dt_str[3:6].lower().title())
                data_var = np.datetime64(
                    datetime.strptime(
                        dt_str, '%d-%b-%Y %H:%M:%S.%f')) + get_data_var(v)
            ...
            # EUMETSAT or DeDop
                ...
                data_var = np.apply_along_axis(
                    lambda vel: np.sqrt(
                        vel[0]**2 + vel[1]**2 + vel[2]**2), 1, v)
            elif is_dart and 'time' in v:
                data_var = np.datetime64(
                    ds.first_measurement_time[:-1]) + pd.to_timedelta(get_data_var(v), 's')
            # CS
            elif is_cs and 'epoch_ref_gate_const' in v:
                data_var = 128.0
            elif is_cs and 'beam_ang_stack' in k:
                angle_start = get_data_var(
                    'dop_angle_start_20_hr_ku') if is_cs_gopr else get_data_var('dop_angle_start_20_ku')
                angle_stop = get_data_var(
                    'dop_angle_stop_20_hr_ku') if is_cs_gopr else get_data_var('dop_angle_stop_20_ku')
                n_beams = get_data_var('stack_number_after_weighting_20_hr_ku').astype(
                    int) if is_cs_gopr else get_data_var('stack_number_after_weighting_20_ku').astype(int)
                ...
                data_var = np.array([func(start, stop, num) for start, stop, num in zip(
                    angle_start, angle_stop, n_beams)])
                ...
                trk_range = get_data_var(
                    'window_del_20_hr_ku') if is_cs_gopr else get_data_var('window_del_20_ku')
                tracker_range_m = trk_range.astype(float) * 1e-9 * CONST_C / 2
                range_m = get_data_var('range_ocean_20_hr_ku') if is_cs_gopr else get_data_var(
                    'range_ocean_20_ku')
                range_res_m = (1 / 320e6) * CONST_C / 2
                data_var = (range_m - tracker_range_m) * \
                    range_res_m * (1 / 320e6) / 1e-9
            ...
            # EUMETSAT
                ...
                num_looks = np.min([get_data_var('num_looks_start_stop'), get_data_var(
                    'num_looks_multilooking')], axis=0).astype(int)
                angle_per_look_rad = (
                    angle_stop - angle_start) / (num_looks - 1)

                def func(start, num, angle_per_look, n_looks_max):
                    arr = np.full(n_looks_max, np.nan)
                    arr[:num] = start + \
                        np.arange(num) * angle_per_look + np.pi / 2
                    return arr

                angs = np.array([func(start,
                                      num,
                                      angle_per_look,
                                      n_looks_max) for start,
                                 num,
                                 angle_per_look in zip(angle_start,
                                                       num_looks,
                                                       angle_per_look_rad)])
                ...
                data_var = np.multiply(
                    get_data_var(v),
                    get_data_var('waveform_scale_factor')[
                        :,
                        np.newaxis])
            ...
            # all
                ...
                mqe_var = [
                    v for k, v in data_var_names.items() if 'mqe' in v][0]
    ...
    if 'wf' in dataset:
        ...
        dataset['entropy'] = np.apply_along_axis(
            ...
            1,
            wf_norm)
        dataset['pulse_peakiness'] = np.nanmax(
            dataset['wf'], axis=1) / np.nansum(dataset['wf'], axis=1)

    # dist2coast
    dataset['dist2coast'] = get_dist_pacioos(
        latarr=np.degrees(
            dataset['lat_rad']), lonarr=np.degrees(
            dataset['lon_rad']), do_interp=do_interp_dist2coast)
    ...
    if is_gpod and 'PASS_DIRECTION_START' in ds.attrs:
        ...
        dataset['ascending'] = bool(
            dataset['lat_rad'][1] > dataset['lat_rad'][0])
...
def get_model_param_obj_from_l1b_data(dataset: dict, ind: int):
    d = {k: (v if isinstance(v, (bool, float, int)) or len(v) <=
             1 else v[ind]) for k, v in dataset.items() if v is not None}
...
def get_nc_src_dest_file_list(l1b_source, nc_dest_dir):
    ...
    if not isinstance(l1b_source, list) and os.path.isdir(l1b_source):
        ...
        src_files_basename = src_files[0].parent.stem if 'measurement' in src_files[
            0].stem and 'SEN6' not in src_files[0].stem else src_files[0].stem

        if 'SEN' in src_files_basename and 'measurement' in src_files_basename:
            dest_files = [
                abs_dest_dir /
                f'{f.name.split(".SEN6.")[0]}.nc' for f in src_files]
        ...
        if 'measurement' in str(src_files[0]):
            dest_files = [abs_dest_dir /
                          (f.parent.name.replace('.SEN6', '') +
                           '.nc') for f in src_files]
        else:
            dest_files = [abs_dest_dir / f.name for f in src_files]
...
def get_nc_dir_df(nc_files):
    ...
    def convert_dt(dt_str): return np.datetime64(
