...
proc_configs = [
    ...
    ((140, 20), 'ffsar_dart', ProcMode.FFSAR,
     2.1, data_vars_dart['l1b_ffsar']),
    # ufsar
...
if __name__ == "__main__":
    ...
    for l1b_l2_posting_rate, procname, mode, T, data_vars in proc_configs:
        ...
        l1b_src_path_dart = COASTAL_FFSAR_PATH / 'processed' / 'l1b' / \
        ...
        l2_destpath = COASTAL_FFSAR_PATH / 'processed' / \
            'l2' / f'{l1b_posting_rate}_{l2_posting_rate}'

        if 'dart' in procname and l1b_l2_path_append:
            data_vars['wf'] = f'power_waveform{l1b_l2_path_append}'

        l1b_files_dart = [f for f in sorted(l1b_src_path_dart.rglob(
            '*.nc')) if '1B' in str(f) and bool(re.search(cycle_pass_pattern, str(f)))]
        l1b_files_eum = [f for f in sorted(l1b_src_path_eum.rglob(
            '*.nc')) if '1B' in str(f) and bool(re.search(cycle_pass_pattern, str(f)))]
        ...
        start_dates = [convert_dt(f.name.split('_')[10])
                       for f in l1b_files_dart]
        end_dates = [convert_dt(f.name.split('_')[11]) for f in l1b_files_dart]
        bbox = [
            (float(
                f.stem.split('_')[21]), float(
                f.stem.split('_')[22]), 0, 360) for f in l1b_files_dart]

        df_dart = pd.DataFrame({'file': l1b_files_dart,
                                'cycle': cycles,
                                'ppass': passes,
        ...
        if is_ffsar:
            l1b_src_type = L1bSourceType.EUM_S6_F04_FFSAR if 'f04' in str(
                l1b_files_dart[0]).lower() else L1bSourceType.EUM_S6_F06_FFSAR
        else:
            l1b_src_type = L1bSourceType.EUM_S6_F04 if 'f04' in str(
                l1b_files_dart[0]).lower() else L1bSourceType.EUM_S6_F06
