...
from pysamosa.data_access import data_vars_s6, data_vars_dart
...
if __name__ == "__main__":
    ...
    for l1b_l2_posting_rate, procname, mode, T, data_vars  in proc_configs:
        ...
        l1b_src_path_dart = COASTAL_FFSAR_PATH / 'processed' / 'l1b' / f"T_{str(T).replace('.','-')}_posting_rate_{l1b_posting_rate}"
        # l1b_src_path_eum = Path('/nfs/DGFI145/A/original_data/sentinel6a/JASON_CS_S6A_L1B_ALT_HR_NTC_F/')
        l1b_src_path_eum = Path('/lfs/DGFI24/bigdata/coastal_ffsar/orig_data/f06/P4_1B_HR_____/')

        # l2_destpath = COASTAL_FFSAR_PATH / f'coral_processed_L2_{l1b_posting_rate}_{l2_posting_rate}'
        l2_destpath = COASTAL_FFSAR_PATH / 'processed' / 'l2' / f'{l1b_posting_rate}_{l2_posting_rate}'

        if 'dart' in procname and l1b_l2_path_append:
            data_vars['wf'] = f'power_waveform{l1b_l2_path_append}'

        l1b_files_dart = [f for f in sorted(l1b_src_path_dart.rglob('*.nc')) if '1B' in str(f) and bool(re.search(cycle_pass_pattern, str(f)))]
        l1b_files_eum = [f for f in sorted(l1b_src_path_eum.rglob('*.nc')) if '1B' in str(f) and bool(re.search(cycle_pass_pattern, str(f)))]
        l1b_files = l1b_files_dart if 'dart' in procname else l1b_files_eum

        logging.info(f'Processing: l1b_l2_posting_rate: {l1b_l2_posting_rate}, len(l1b_files):{len(l1b_files)}, l1b_path:{l1b_src_path_dart} , l2_path:{l2_destpath}')
        ...
        bbox = [(float(f.stem.split('_')[21]), float(f.stem.split('_')[22]), 0, 360) for f in l1b_files_dart]

        df_dart = pd.DataFrame({'file': l1b_files_dart, 'cycle': cycles, 'ppass': passes, 'start_date': start_dates, 'end_date': end_dates, 'dom': bbox})
        ...
        if is_ffsar:
            l1b_src_type = L1bSourceType.EUM_S6_F04_FFSAR if 'f04' in str(l1b_files_dart[0]).lower() else L1bSourceType.EUM_S6_F06_FFSAR
        else:
            l1b_src_type = L1bSourceType.EUM_S6_F04 if 'f04' in str(l1b_files_dart[0]).lower() else L1bSourceType.EUM_S6_F06
        ...
        rp = RetrackerProcessor(l1b_source=l1b_files, l1b_data_vars=data_vars,
                                bbox=bbox,
                                rp_sets=rp_sets,
