...
if is_slurm:
    if 'SLURM_ARRAY_TASK_COUNT' in os.environ:
        ...
        slurm_n_processes_per_job = int(
            os.environ['SLURM_NPROCS']) if 'SLURM_NPROCS' in os.environ else 1
...
rr_l1b_src_dir = (Path('/nfs/') if is_linux else Path('U:/')) / \
    'DGFI145/C/seastate_cci/round-robin/satellite/' / l1b_base_path
...
def get_l1b_src_files(*, nc_dest_path, skip_if_exists=True):
    l1b_files = [f for f in sorted(Path(rr_l1b_src_dir).rglob(
        '*.nc')) if bool(re.search(f'(track_){rr_track_nums}', str(f)))]

    if is_slurm:
        # remove files from list if they already exist
        if skip_if_exists:
            l1b_files = [f for f in l1b_files if not (
                nc_dest_path / str(f.relative_to(rr_l1b_src_dir))).exists()]
        ...
        l1b_files = list(split(l1b_files, slurm_n_total_processes))[
            slurm_abs_procid]
...
if __name__ == "__main__":
    if (rbt == RetrackerBaseType.SAMPLUS or rbt ==
            RetrackerBaseType.SAMPLUSPLUS) and l1bsrc_type == L1bSourceType.EUM_S3:
        raise RuntimeError(
    ...
    if 'l1b_files' not in locals():
        l1b_files = get_l1b_src_files(
            nc_dest_path=rp_sets.nc_dest_dir,
            skip_if_exists=rp_sets.skip_if_exists)
