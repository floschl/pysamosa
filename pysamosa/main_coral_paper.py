...
if is_slurm:
    if 'SLURM_ARRAY_TASK_COUNT' in os.environ:
        ...
        slurm_n_processes_per_job = int(os.environ['SLURM_NPROCS']) if 'SLURM_NPROCS' in os.environ else 1
    ...
    print(f'slurm_abs_procid/slurm_n_total_processes: ({slurm_abs_procid}/{slurm_n_total_processes})')

process_rr_files = True

top_coastal_tracks = ['371', '478', '592', '074', '463', '207', '508', '250', '514', '491', '741', '355', '181', '285', '319', '493', '530', '115', '703', '226', '656', '513', '084', '099', '646', '142', '011', '393', '695', '474']
...
l1bsrc_type, rbt, preset = (
    L1bSourceType.GPOD, RetrackerBaseType.SAMPLUS, SettingsPreset.CORALv1,
)

# round robin files config
if rbt == RetrackerBaseType.SAM:
    l1b_base_path = 's3a_sr_1_sra_bs' if l1bsrc_type is L1bSourceType.EUM_S3 else 's3a_sr_1_sra_bs_gpod_sam_wfs'
...
def get_l1b_src_files(*, nc_dest_path, skip_if_exists=True):
    l1b_files = [f for f in sorted(Path(rr_l1b_src_dir).rglob('*.nc')) if bool(re.search(f'(track_){rr_track_nums}', str(f)))]

    if is_slurm:
        # remove files from list if they already exist
        if skip_if_exists:
            l1b_files = [f for f in l1b_files if not (nc_dest_path / str(f.relative_to(rr_l1b_src_dir))).exists()]

        def split(a, n):
            k, m = divmod(len(a), n)
            return (a[i * k + min(i, m):(i + 1) * k + min(i + 1, m)] for i in range(n))

        # split file list into slurm_n_procs chunks of files, take slurm_procid's chunk
        l1b_files = list(split(l1b_files, slurm_n_total_processes))[slurm_abs_procid]

    return l1b_files[::-1]

if __name__ == "__main__":
    if (rbt == RetrackerBaseType.SAMPLUS or rbt == RetrackerBaseType.SAMPLUSPLUS) and l1bsrc_type == L1bSourceType.EUM_S3:
        raise RuntimeError('RetrackerBaseType SAM+/SAM++ and L1bSourceType.EUMETSAT is not compatible. ')

    nc_dest_path = dest_path_base / (l1bsrc_type.value + '_' + rbt.value)

    rp_sets, retrack_sets, fitting_sets, wf_sets, sensor_sets = get_default_base_settings(retracker_basetype=rbt, settings_preset=preset, l1b_src_type=l1bsrc_type)
    ...
    if 'l1b_files' not in locals():
        l1b_files = get_l1b_src_files(nc_dest_path=rp_sets.nc_dest_dir, skip_if_exists=rp_sets.skip_if_exists)
    ...
    rp = RetrackerProcessor(l1b_source=l1b_files, l1b_data_vars=l1b_data_vars,
                            rp_sets=rp_sets,
