...
def calc_misfit(wf1, wf2, inds_only=None, exclude_inds=None):
    ...
    exclude_inds = np.asarray(
        exclude_inds) if exclude_inds is not None else None
...
def oversample_wf(wf, int_oversampling_fac):
    ...
    wf_oversampled[-(int_oversampling_fac)
                     :] = wf_oversampled[-(int_oversampling_fac)]
...
class SamosaRetracker():
    ...
    def __init__(self, retracker_basetype: RetrackerBaseType,
        ...
        self.B_r_Hz_oversampled = self.sensor_sets.B_r_Hz * \
            self.wf_sets.zp_oversampling_factor * self.wf_sets.internal_oversampling_factor
    ...
    def apply_fit_mask(*, wf, wf_sets, retrack_sets):
        ...
        first_bin = retrack_sets.Wf_Fit_First_Bin * \
            os_factor if retrack_sets.Wf_Fit_First_Bin != -1 else None
        last_bin = retrack_sets.Wf_Fit_Last_Bin * \
            os_factor if retrack_sets.Wf_Fit_Last_Bin != -1 else None
    ...
    def init_processing(self, l1b_data_single, *, disable_oversampling=False):
        ...
        if int_oversampling_fac > 1 and not disable_oversampling:
            Wf_in = oversample_wf(
                Wf_in, self.wf_sets.internal_oversampling_factor)
        ...
        if self.retrack_sets.n_effective_looks != 0 and 'stack' in l1b_data_single:
            ...
            inds_to_sum = np.arange(
                first_look,
                first_look +
                self.retrack_sets.n_effective_looks)
        ...
        if (Wf_in.size) != self.wf_sets.np:
            raise RuntimeError(
                f'Wf_in must be of length Np(={self.wf_sets.np})')
        ...
        if 'dynamic_fg_epoch' in l1b_data_single:
            self.dynamic_fg_epoch = l1b_data_single['dynamic_fg_epoch'] * \
                int_oversampling_fac
        ...
        if 's6' in self.sensor_sets.sensor_type.value and self.retrack_sets.auto_detect_s6_rmc:
            is_rmc_active = np.mean(
                self.Wf_norm[-self.wf_sets.np // 3:]) < 1e-3
    ...
    def _fun_model(p, *args):
        ...
        try:
            if fitting_params.fit_nu_instead_of_swh:
                ...
                if retrack_sets.Disable_ML_Flag:
                    wf_model = sm.get_waveform_singlelook(
                        Pu=p[2], Hs=p[1], t0_ns=epoch_init, fa_Hz=0, model_params=model_params)
                else:
                    wf_model = sm.get_waveform_multilook(
        ...
        if retrack_sets.interference_masking and not fitting_params.disable_interference_masking:
            ...
            interference_inds = [
                ei for ei in interference_inds if ei not in le_inds]
    ...
    def _run_fitting_process(
            ...
            fitting_params: FittingParameters = None):
        fitting_params = fitting_params if fitting_params is not None else FittingParameters()

        fg_gate = self.dynamic_fg_epoch if self.dynamic_fg_epoch else np.argmax(
            self.Wf_norm)
        ...
        res_ls = least_squares(fun=SamosaRetracker._fun_model,
                               ...
                               method='trf',
                               args=(
                                   {**_fun_model_arg[0], **{'return_diff': True}},),
        ...
        wf_opt = self.res_fit_info['wf_opt'] if 'wf_opt' in self.res_fit_info else np.zeros(
            len(self.Wf_norm))
        ...
        if self.retrack_sets.subwaveform_mode:
            misfit_selective = calc_misfit(wf_opt[:res_fit['max_le_gate'] +
                                                  self.retrack_sets.subwaveform_n_gates_after_le], self.Wf_norm[:res_fit['max_le_gate'] +
                                                                                                                self.retrack_sets.subwaveform_n_gates_after_le])
        else:
            misfit_selective = calc_misfit(
                wf_opt, self.Wf_norm, exclude_inds=self.res_fit_info['interference_inds'])
    ...
    def fit_wf(self, l1b_data_single, model_params: ModelParameter):
        ...
        epoch_ref_gate = model_params.epoch_ref_gate * \
            self.wf_sets.internal_oversampling_factor
        self.t = np.arange(-epoch_ref_gate, -epoch_ref_gate +
                           self.wf_sets.np) * self.dtau
        ...
        if self.retrack_sets.interference_masking_second_retracking_step:
            ...
            self.retrack_sets.interference_masking_swh_max = np.min(
                [res_fit["swh"] + upper_swh_margin, self.fitting_sets.Fit_Var_2_MinMax_Hs[1]])
            res_fit = self._run_fitting_process(model_params)
        ...
        if self.retrack_sets.second_retracking_step_samplus:
            if 'entropy' in dir(self) and 'pulse_peakiness' in dir(self) and is_two_step_retracking_active_samplus(
                    entropy=self.entropy, pulse_peakiness=self.pulse_peakiness, misfit=res_fit['misfit']):
                n_iter_first = res_fit['n_iter']
