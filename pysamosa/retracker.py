...
from pysamosa.common_types import ModelSettings, ModelParameter, RetrackerSettings, FittingSettings, \
    WaveformSettings, FittingParameters, L1bSourceType
from pysamosa.common_types import RetrackerBaseType, SensorSettings, SensorType
...
def calc_misfit(wf1, wf2, inds_only=None, exclude_inds=None):
    ...
    if exclude_inds is not None and exclude_inds.size:
        ...
        misfit = 100 * np.sqrt(np.mean((wf1[inds_only] - wf2[inds_only]) ** 2))  # according to Dinardo, 2020, eq. 3.52
    else:
        misfit = 100 * np.sqrt(np.mean((wf1 - wf2)**2))  # according to Dinardo, 2020, eq. 3.52
...
def longest_consecutive_run(data):
    return sorted(np.split(data, np.where(np.diff(data) != 1)[0]+1), key=len, reverse=True)[0]
...
def oversample_wf(wf, int_oversampling_fac):
    ...
    wf_oversampled[-(int_oversampling_fac):] = wf_oversampled[-(int_oversampling_fac)]  #replace last four values by the last valid ones
...
class SamosaRetracker():
    ...
    def __init__(self, retracker_basetype : RetrackerBaseType,
        ...
        self.model_sets =  ModelSettings.get_default_sets(st=sensor_sets.sensor_type, retracker_basetype=retracker_basetype,
        ...
        self.B_r_Hz_oversampled = self.sensor_sets.B_r_Hz * self.wf_sets.zp_oversampling_factor * self.wf_sets.internal_oversampling_factor
    ...
    def get_wf_max(wf, retrack_sets, fg_epoch=0):
        if retrack_sets.normalise_wf_by_fg_region:
            wf_max = get_region_max(wf, fg_epoch, retrack_sets.normalise_wf_by_fg_region)
        else:
    ...
    def apply_fit_mask(*, wf, wf_sets, retrack_sets):
        ...
        first_bin = retrack_sets.Wf_Fit_First_Bin * os_factor if retrack_sets.Wf_Fit_First_Bin != -1 else None
        last_bin = retrack_sets.Wf_Fit_Last_Bin * os_factor if retrack_sets.Wf_Fit_Last_Bin != -1 else None
    ...
    def init_processing(self, l1b_data_single, *, disable_oversampling=False):
        ...
        # perform an internal oversampling of the waveform to ease fitting procedure
        ...
        if self.retrack_sets.n_effective_looks != 0 and 'stack' in l1b_data_single:
            ...
            inds_to_sum = np.arange(first_look, first_look + self.retrack_sets.n_effective_looks)
        ...
        elif self.retrack_sets.n_effective_looks != 0 and 'stack' not in l1b_data_single:
            raise RuntimeError('No stack data exists although n_effective_looks param is configured!')

        if np.isnan(Wf_in).any():
            raise RuntimeError(f'Wf_in contains NaN values.')
        ...
        if len(l1b_data_single['beam_ang_stack_rad']) and np.all(np.isnan(l1b_data_single['beam_ang_stack_rad'])):
            raise RuntimeError(f'Invalid beam_ang_stack_rad variable. ')

        self.dynamic_fg_epoch = None
        if 'dynamic_fg_epoch' in l1b_data_single:
            self.dynamic_fg_epoch = l1b_data_single['dynamic_fg_epoch'] * int_oversampling_fac

        if self.retrack_sets.normalise_wf_by_fg_region and 'dynamic_fg_epoch' not in l1b_data_single:
        ...
        # I2: Check altimeter flags to decide if conditions are met to proceed with fitting
        ...
        Wf_in_TN_part = Wf_in[(self.retrack_sets.Wf_TN_First * os_factor):(self.retrack_sets.Wf_TN_Last * os_factor) + 2]

        # in case of SAMPLUS-L1B waveforms, add 4.84 dB to thermal noise, Dinardo2020 3.2.3
        if (self.retracker_basetype == RetrackerBaseType.SAMPLUS or self.retracker_basetype == RetrackerBaseType.SAMPLUSPLUS)\
                and 'thermal_noise' in l1b_data_single and not np.isnan(l1b_data_single['thermal_noise']):
        ...
        if self.retrack_sets.Wf_Norm_Flag:
            if self.dynamic_fg_epoch:
                self.Wf_max = SamosaRetracker.get_wf_max(Wf_in, self.retrack_sets, fg_epoch=self.dynamic_fg_epoch)
            else:
                self.Wf_max = SamosaRetracker.get_wf_max(Wf_in, self.retrack_sets)
            # The value of Wf_max needs to be checked against TN to determine if normalisation can proceed.
        ...
        if 's6' in self.sensor_sets.sensor_type.value and self.retrack_sets.auto_detect_s6_rmc:
        ...
        if np.allclose(self.Wf_norm, 0.0):
            raise RuntimeError(f'Wf_in are all-zero after normalisation. ')
    ...
    def _get_least_square_weights(self):
        ...
        try:
            le_inds = detect_leading_edge(wf, fg_epoch=self.dynamic_fg_epoch, normalise_wf_by_fg_region=self.retrack_sets.normalise_wf_by_fg_region)
    ...
    def _fun_model(p, *args):
        ...
        logging.debug(f'Generating SAMOSA waveform with params: P_u={p[2]}, Hs={p[1]}, t0_ns={epoch_init}')

        # return diff wf_model - wf if requested
        do_return_diff = True if 'return_diff' in kwargs and kwargs['return_diff'] else False

        sm = SamosaModel(
            model_sets=_self.model_sets,
            sensor_sets=_self.sensor_sets,
            wf_sets=_self.wf_sets,  # reset the internal_oversampling_factor and generates a non-oversampled model
        )
        ...
        try:
            if fitting_params.fit_nu_instead_of_swh:
                if retrack_sets.Disable_ML_Flag:
                    wf_model = sm.get_waveform_singlelook(Pu=p[2], Hs=fitting_params.swh_first_step, t0_ns=epoch_init, fa_Hz=0, nu=p[1], model_params=model_params)
                else:
                    wf_model = sm.get_waveform_multilook(Pu=p[2], Hs=fitting_params.swh_first_step, t0_ns=epoch_init, nu=p[1], model_params=model_params)
            else:
                if retrack_sets.Disable_ML_Flag:
                    wf_model = sm.get_waveform_singlelook(Pu=p[2], Hs=p[1], t0_ns=epoch_init, fa_Hz=0, model_params=model_params)
                else:
                    wf_model = sm.get_waveform_multilook(Pu=p[2], Hs=p[1], t0_ns=epoch_init, model_params=model_params)
        except RuntimeWarning as e:
            logging.warning(f'Warning in generating multilooked waveform: {" ".join(e.args)}')
            wf_model = np.zeros(_self.wf_sets.np)

        if np.allclose(wf_model, 0.0):
            logging.warning(f'Calculated multilooked waveform is zero. Params: P_u={p[2]}, Hs={p[1]}, t0_ns={epoch_init}')
        ...
        if retrack_sets.interference_masking and not fitting_params.disable_interference_masking:
            ...
            interference_mask, interference_inds = sm.get_waveform_interference_mask(
                wf=wf, t0_ns=epoch_init, model_params=model_params,
        ...
        if _self.retrack_sets.subwaveform_mode:
            max_le_gate = get_region_argmax(wf, region_center=_self.dynamic_fg_epoch, n_before_after=_self.retrack_sets.normalise_wf_by_fg_region)
        ...
        res = SamosaRetracker.apply_fit_mask(wf=res, wf_sets=_self.wf_sets, retrack_sets=_self.retrack_sets)

        return res

    def _run_fitting_process(self, model_params: ModelParameter, fitting_params: FittingParameters = None):
        fitting_params = fitting_params if fitting_params is not None else FittingParameters()

        fg_gate = self.dynamic_fg_epoch if self.dynamic_fg_epoch else np.argmax(self.Wf_norm)
        ...
        # epoch/t0
        minmax_epoch = fitting_params.minmax_epoch if fitting_params.minmax_epoch else self.fitting_sets.Fit_Var_1_MinMax_epoch
        init_epoch = fitting_params.init_epoch[0] if fitting_params.init_epoch else self.t[fg_gate] / 1e-9
        ...
        if fitting_params.fit_nu_instead_of_swh:
            ...
            minmax_hs_or_nu = fitting_params.minmax_swh if fitting_params.minmax_swh else self.fitting_sets.Fit_Var_2_MinMax_Hs
            init_hs_or_nu = fitting_params.init_swh if fitting_params.init_swh else self.fitting_sets.Fit_Var_2_Init_Hs

        # Pu
        minmax_pu = fitting_params.minmax_pu if fitting_params.minmax_pu else self.fitting_sets.Fit_Var_3_MinMax_Pu
        init_pu = fitting_params.init_pu if fitting_params.init_pu else self.fitting_sets.Fit_Var_3_Init_Pu

        # nu
        minmax_nu = fitting_params.minmax_nu if fitting_params.minmax_nu else self.fitting_sets.Fit_Var_5_MinMax_nu
        init_nu = fitting_params.init_nu if fitting_params.init_nu else self.fitting_sets.Fit_Var_5_Init_nu
        ...
        Fit_Var_Bounds = ([epoch_min, minmax_hs_or_nu[0],
                           minmax_pu[0]],
                          [epoch_max, minmax_hs_or_nu[1],
                           minmax_pu[1]])

        max_nfev = self.fitting_sets.Fit_MaxIter

        res_ls = least_squares(fun=SamosaRetracker._fun_model,
                               x0=Fit_Var_Init,
        ...
        if fitting_params.fit_nu_instead_of_swh:
            res_fit = {'swh': fitting_params.swh_first_step,
                       'epoch_ns': res_ls.x[0],
                       'Pu': res_ls.x[2],
                       'nu': res_ls.x[1],
                       'n_iter': res_ls.nfev,
        ...
        self.res_fit_info['wf_opt'] = SamosaRetracker.apply_fit_mask(wf=self.res_fit_info['wf_opt'], wf_sets=self.wf_sets, retrack_sets=self.retrack_sets)

        # take fitted waveform that was generated at the very last step
        wf_opt = self.res_fit_info['wf_opt'] if 'wf_opt' in self.res_fit_info else np.zeros(len(self.Wf_norm))
        ...
        if self.retrack_sets.subwaveform_mode:
            misfit_selective = calc_misfit(wf_opt[:res_fit['max_le_gate'] + self.retrack_sets.subwaveform_n_gates_after_le],
                                           self.Wf_norm[:res_fit['max_le_gate'] + self.retrack_sets.subwaveform_n_gates_after_le])
        else:
            misfit_selective = calc_misfit(wf_opt, self.Wf_norm, exclude_inds=self.res_fit_info['interference_inds'])
    ...
    def fit_wf(self, l1b_data_single, model_params: ModelParameter):
        ...
        epoch_ref_gate = model_params.epoch_ref_gate * self.wf_sets.internal_oversampling_factor
        self.t = np.arange(-epoch_ref_gate, -epoch_ref_gate + self.wf_sets.np) * self.dtau
        ...
        if self.retrack_sets.interference_masking_second_retracking_step:
            ...
            logging.debug(f'Starting second iteration step with updated interference_masking_swh_max={res_fit["swh"]}. ')
            upper_swh_margin = 2.0
            self.retrack_sets.interference_masking_swh_max = np.min([res_fit["swh"] + upper_swh_margin, self.fitting_sets.Fit_Var_2_MinMax_Hs[1]])
            res_fit = self._run_fitting_process(model_params)
        ...
        if self.retrack_sets.second_retracking_step_samplus:
            if 'entropy' in dir(self) and 'pulse_peakiness' in dir(self) and is_two_step_retracking_active_samplus(entropy=self.entropy, pulse_peakiness=self.pulse_peakiness, misfit=res_fit['misfit']):
                n_iter_first = res_fit['n_iter']

                fitting_params = FittingParameters(fit_nu_instead_of_swh=True, swh_first_step=res_fit['swh'],
                                                   disable_interference_masking=False,
                                                   )
                logging.debug(f'Starting second iteration step, taking SWH={res_fit["swh"]} from first step. ')
                res_fit = self._run_fitting_process(model_params=model_params, fitting_params=fitting_params)

                res_fit['n_iter'] = (n_iter_first, res_fit['n_iter'])

        logging.debug(f'Finished fitting (n_iter={res_fit["n_iter"]}). Found params: swh={res_fit["swh"]:.2f}m, '
