...
class RetrackerProcessor():
    ...
    def __init__(self, *, l1b_source, l1b_data_vars,
        ...
        self.rp_sets = RetrackerProcessorSettings(
            retracker_basetype=self.rt) if rp_sets is None else rp_sets
        ...
        self.bbox = bbox if bbox is not None else [
            None for i in range(len(self.l1b_src_files))]
        ...
        logging.info(
            ...
            (
                ', '.join(
                    [
                        f'{k}: {v}' for k,
                        v in nc_attrs_kw.items()]) if nc_attrs_kw is not None else ''))

        self.retrack_sets = RetrackerSettings(
            retracker_basetype=self.rt) if retrack_sets is None else retrack_sets
        self.fitting_sets = FittingSettings(
            retracker_basetype=self.rt) if fitting_sets is None else fitting_sets
        self.sensor_sets = sensor_sets if sensor_sets is not None else SensorSettings()
        self.wf_sets = wf_sets if wf_sets is not None else WaveformSettings.get_default_src_type(
    ...
    def _write_fit_results(self, rel_ind, res_fit):
        if res_fit:
            ...
            self.output_l2.range[rel_ind] = self.l1b_data['tracker_range_m'][rel_ind] + (
    ...
    def process(self):
        for nc_src_file, nc_dest_filepath, bbox in zip(
            ...
            try:
                ...
                grps = [
                    g for g in list(
                        netCDF4.Dataset(nc_src_file).groups) if 'data' in g]
                ...
                if self.rp_sets.do_dynamic_fg_epoch:
                    ...
                    logging.info(
                        f'Finished calculating dynamic_first_guess_epochs.')
                ...
                abs_inds_process = (
                    self.rp_sets.n_offset +
                    np.arange(
                        self.output_l2.swh.shape[0]))[mask_inds_process]
                rel_inds_process = mask_inds_process.nonzero()[0]

                # Start processing
                logging.info(
                    f'Started processing of L1B dataset {nc_base_id}... ')
                if self.rp_sets.n_procs == 1:
                    for rel_ind, abs_ind in zip(
                            rel_inds_process, abs_inds_process):
                        res_fit = RetrackerProcessor._process_single(
                            retracker_obj=self.retracker, l1b_data_single=get_subset_dataset(
                                self.l1b_data, rel_ind), model_params=get_model_param_obj_from_l1b_data(
                                self.l1b_data, rel_ind), abs_ind=abs_ind)
                    ...
                    with futures.ProcessPoolExecutor(max_workers=self.rp_sets.n_procs) as pool:
                        for rel_ind, res_fit in zip(
                            rel_inds_process, pool.map(
                                RetrackerProcessor._process_single, repeat(
                                    self.retracker), gen_subset_dataset(
                                    self.l1b_data, rel_inds_process), gen_model_param_obj_from_l1b_data(
                                    self.l1b_data, rel_inds_process), abs_inds_process, chunksize=20)):
                            self._write_fit_results(rel_ind, res_fit)

                logging.info(
                    f'Finished processing of L1B dataset {nc_base_id}. ')
                ...
                if self.rp_sets.do_generate_results_pdf:
                    ...
                    axs_data[1].plot(
                        record_inds, self.output_l2.range, linewidth=0.5)
                    axs_data[1].set_xlim(record_inds.min(), record_inds.max())
                    axs_data[1].grid()

                    fig_scatter, ax_scatter = scatter_map(
                        self.output_l2.latitude, self.output_l2.longitude, self.output_l2.swh, title=self.rt.value)

                    # printout
                    figs_list = [fig_data, fig_scatter]
                    pdf_filepath = nc_dest_dir / \
    ...
    def reduce_l2(self):
        if self.rp_sets.reduce_l2_factor > 1:
            reduce_offset = self.rp_sets.reduce_l2_factor // 2 if (
                self.rp_sets.reduce_l2_factor %
                2) != 0 else self.rp_sets.reduce_l2_factor // 2 - 1
            ds_reduced = self.get_init_output_l2(
                reduce_factor=self.rp_sets.reduce_l2_factor,
                reduce_offset=reduce_offset)
            is_ufsar = not self.retrack_sets.fit_zero_doppler
            len_seq = len(self.output_l2.record_ind)

            for rel_ind, rec_ind in enumerate(
                    np.unique(self.output_l2.record_ind)):
                ...
                if all(~abs_mask_good_recs):  # if all are bad estimates
                    ...
                    ds_reduced.swh[rel_ind] = avg_func(
                        self.output_l2.swh[mask_sel])
                    ds_reduced.swh_qual[rel_ind] = 1.0
                    ds_reduced.epoch[rel_ind] = avg_func(
                        self.output_l2.epoch[mask_sel])
                    ds_reduced.range[rel_ind] = avg_func(
                        self.output_l2.range[mask_sel])
                    ds_reduced.Pu[rel_ind] = avg_func(
                        self.output_l2.Pu[mask_sel])
                else:
                    avg_func = np.nanmean
                    ds_reduced.swh[rel_ind] = avg_func(
                        self.output_l2.swh[abs_mask_good_recs])
                    ds_reduced.swh_qual[rel_ind] = float(
                        self.output_l2.swh_qual[abs_mask_good_recs]) if np.isscalar(
                        self.output_l2.swh_qual[abs_mask_good_recs]) else 0.0
                    ds_reduced.epoch[rel_ind] = avg_func(
