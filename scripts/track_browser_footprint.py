...
for nc_filename in nc_filenames:
    ...
    grps = [g for g in list(netCDF4.Dataset(nc_filename).groups)
            if 'data_20' in g or 'data_140' in g]
    nc_grp = grps[0] + '/ku' if len(grps) > 0 else ''

    nc_basename = nc_filename.stem if 'measurement' not in str(
        nc_filename) else nc_filename.parent.stem

    if 'dart' in str(nc_filename):
        cycle, ppass = int(
            nc_basename.split('_')[13]), int(
            nc_basename.split('_')[14])
    elif '_2_' in nc_basename:
        cycle, ppass = int(
            nc_basename.split('_')[8]), int(
            nc_basename.split('_')[9])
    ...
    with xr.open_dataset(nc_filename, decode_times=False, group=nc_grp) as dstmp:
        if is_s6:
            data_vars = {
                'lat_rad': [
                    k for k in list(
                        dstmp.coords.keys()) if 'lat' in k][0], 'lon_rad': [
                    k for k in list(
                        dstmp.coords.keys()) if 'lon' in k][0], 'time': [
                    k for k in list(
                        dstmp.coords.keys()) if 'time' in k][0], }
        elif is_s3:
            data_vars = {
                'lat_rad': [
                    k for k in list(
                        dstmp.coords.keys()) if 'lat' in k and 'sar' in k][0], 'lon_rad': [
                    k for k in list(
                        dstmp.coords.keys()) if 'lon' in k and 'sar' in k][0], 'time': [
                    k for k in list(
                        dstmp.coords.keys()) if 'time' in k and 'sar' in k][0], }
    ...
    df = pd.DataFrame({k: v for k,
                       v in ds.items() if k in ['time',
                                                'lat_rad',
                                                'lon_rad',
                                                'alt_m',
                                                'dist2coast',
    ...
    def calc_bearings(lat, lon):
        ...
        for i in range(lat.size):
            if i != (lat.size - 1):
                ...
                Y = np.cos(lat0) * np.sin(lat1) - np.sin(lat0) * \
                    np.cos(lat1) * np.cos(dL)
    ...
    xp = df['footprint_size_x'].values[:,
                                       np.newaxis] * rect_points_xy[np.newaxis]
    yp = df['footprint_size_y'].values[:,
                                       np.newaxis] * rect_points_xy[np.newaxis]
    ...
    for i in range(xp.shape[0]):
        ...
        # rotate points
        points_rotated = np.apply_along_axis(lambda p_i: rotate(
            p_i, angle_rad=-bearings[i]), axis=0, arr=np.vstack([x, y]))
        xs[i, :] = points_rotated[0, :]
        ys[i, :] = points_rotated[1, :]

    xs = df['mercator_x'].values[:, np.newaxis] * \
        np.ones((1, xs.shape[1])) + xs
    ys = df['mercator_y'].values[:, np.newaxis] * \
        np.ones((1, ys.shape[1])) + ys
