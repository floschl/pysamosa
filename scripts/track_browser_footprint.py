...
for nc_filename in nc_filenames:
    ...
    grps =[g for g in list(netCDF4.Dataset(nc_filename).groups) if 'data_20' in g or 'data_140' in g]
    nc_grp = grps[0] + '/ku' if len(grps) > 0 else ''

    nc_basename = nc_filename.stem if not 'measurement' in str(nc_filename) else nc_filename.parent.stem
    ...
    with xr.open_dataset(nc_filename, decode_times=False, group=nc_grp) as dstmp:
        if is_s6:
            ...
            data_vars = {
                'lat_rad': [k for k in list(dstmp.coords.keys()) if 'lat' in k and 'sar' in k][0],
                'lon_rad': [k for k in list(dstmp.coords.keys()) if 'lon' in k and 'sar' in k][0],
                'time': [k for k in list(dstmp.coords.keys()) if 'time' in k and 'sar' in k][0],
            }

    if 'model' in str(nc_filename):
        data_vars['angle_of_approach_to_coast'] = 'angle_of_approach_to_coast'

    ds = _read_dataset_vars_from_ds(nc_filename=nc_filename, data_var_names=data_vars, group=nc_grp)
    ...
    df = pd.DataFrame({k:v for k,v in ds.items() if k in ['time', 'lat_rad', 'lon_rad', 'alt_m', 'dist2coast', 'distance_to_coast', 'record_inds', 'angle_of_approach_to_coast']})
    ...
    def calc_bearings(lat, lon):
        ...
        for i in range(lat.size):
            if i != (lat.size -1):
                ...
                Y = np.cos(lat0)*np.sin(lat1) - np.sin(lat0)*np.cos(lat1) * np.cos(dL)
    ...
    ####
    # hack because the hover area of Rect is shift (it is a bug: https://github.com/bokeh/bokeh/issues/9752)
    ...
    for i in range(xp.shape[0]):
        ...
        # rotate points
        points_rotated = np.apply_along_axis(lambda p_i: rotate(p_i, angle_rad=-bearings[i]), axis=0, arr=np.vstack([x, y]))
    ...
    # Display in notebook
    output_file(Path(tempfile.gettempdir()) / 'track_browser.html', title=f'{cycle}/{ppass}')
