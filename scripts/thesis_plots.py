...
from pysamosa.data_access import _read_dataset_vars_from_ds, get_subset_dataset, get_model_param_obj_from_l1b_data, data_vars_s6
...
fig_list = {}
fig_scale_factor = 2
figwidth_textwidth = fig_scale_factor * m2t.default_figsize_in[0]
figheight_textwidth = fig_scale_factor * m2t.default_figsize_in[1]
default_figsize = [figwidth_textwidth, figheight_textwidth]
default_dpi = 400
default_subplot_name_kw = {'xycoords': 'axes fraction', 'xy': (0.02, 0.96), 'textcoords': 'offset pixels', 'xytext': (0,0) , 'horizontalalignment': 'left', 'verticalalignment': 'top', 'fontweight': 'bold', 'color': 'black', 'bbox': dict(boxstyle='square,pad=.2', facecolor='white', edgecolor='none', alpha=0.7, zorder=10)}
...
def gen_multilooked_vary_swh():
    ...
    for s in swh:
        wf = sm.get_waveform_multilook(Pu=Pu, Hs=s, t0_ns=t_0, nu=nu, model_params=model_params)
        # wf = wf + np.random.randn(len(wf)) * 0.01 * np.nanmax(wf)  # NOISE ADDITION (not in the SAMOSA documentation )
        print(f'SWH={s} m, argmax={wf.argmax()}, retracking_point={np.abs(wf - 0.8422).argsort()[1]}')
        ax.plot(wf, label=f'SWH={s:.2f} m', linewidth=1, color=next(cols_it))

    # plot epoch reference gate vertical line

    wf_2 = sm.get_waveform_multilook(Pu=Pu, Hs=2.0, t0_ns=t_0, nu=nu, model_params=model_params)

    ax.axvline(epoch_ref_gate, linestyle='--', linewidth='1.0', label='Epoch reference gate', color=next(cols_it))
    ...
    ax.annotate('', xy=(x_retracking_point,level_retracking_point), xytext=(epoch_ref_gate, level_retracking_point), arrowprops=dict(arrowstyle='<|-|>', shrinkA=0, shrinkB=0, color='black'))
    ax.annotate('epoch', xy=(x_retracking_point + (epoch_ref_gate-x_retracking_point)/2, level_retracking_point + 0.02), horizontalalignment='center')

    x_pu,y_pu = 25,1.0
    ax.annotate('', xy=(x_pu,0.0), xytext=(x_pu, y_pu), arrowprops=dict(arrowstyle='<|-|>', shrinkA=0, shrinkB=0, color='black'))
...
def plot_single_retrackings():
    fig, axs = plt.subplots(1, 4, figsize=[figwidth_textwidth, figwidth_textwidth/4], dpi=default_dpi, sharey='row')
    axs = axs.ravel()
    ax_it = iter(axs)
    ...
    list_scenarios = [
        # open ocean
        (46720, Path('/nfs/DGFI145/C/work_flo/coastal_ffsar/orig_data/f06/P4_1B_HR_____/S6A_P4_1B_HR______20210413T182810_20210413T192223_20220514T131950_3253_015_213_106_EUM__REP_NT_F06.SEN6/measurement.nc')),
        # coastal: peak after leading edge
        (8586, Path('/nfs/DGFI145/C/work_flo/coastal_ffsar/orig_data/f06/P4_1B_HR_____/S6A_P4_1B_HR______20210502T223038_20210502T232547_20220505T002438_3309_017_196_098_EUM__REP_NT_F06.SEN6/measurement.nc')),
        # coastal: peak after leading edge
        (8581, Path('/nfs/DGFI145/C/work_flo/coastal_ffsar/orig_data/f06/P4_1B_HR_____/S6A_P4_1B_HR______20210502T223038_20210502T232547_20220505T002438_3309_017_196_098_EUM__REP_NT_F06.SEN6/measurement.nc')),
                # coastal: multipeak scenario
        (46624, Path('/nfs/DGFI145/C/work_flo/coastal_ffsar/orig_data/f06/P4_1B_HR_____/S6A_P4_1B_HR______20210413T182810_20210413T192223_20220514T131950_3253_015_213_106_EUM__REP_NT_F06.SEN6/measurement.nc')),
    ]
    datavars_l1b = data_vars_s6['l1b']
    # datavars_l2 = data_vars_eumetsat_s6['l2']

    l1b_src_type, retracker_basetype, settings_preset = (L1bSourceType.EUM_S6_F06, RetrackerBaseType.SAM, SettingsPreset.NONE)
    ...
    for ind, file_l1b in list_scenarios:
        ...
        l1b_data = _read_dataset_vars_from_ds(nc_filename=file_l1b, data_var_names=datavars_l1b, n_offset=ind, n_inds=2, group=grp)
        ...
        ncfile_l1bs = file_l1b
        model_params = get_model_param_obj_from_l1b_data(l1b_data, ind=0)

        sr = retracker.SamosaRetracker(retracker_basetype=retracker_basetype,
        ...
        ax.plot(l1b['wf'] / np.max(l1b['wf']), color=okabeito_colorblind_colors['blue'], label=f'measured waveform')
        ax.plot(res_fit['wf_opt'], color=okabeito_colorblind_colors['orange'],
        ...
        legend_fontsize = 6
        ax.legend(**{**default_legend_kw,**{'fontsize': legend_fontsize}},
                  title=f'S6-MF cycle {int(file_l1b.parent.stem.split("_")[13])}, pass {int(file_l1b.parent.stem.split("_")[14])},\n'
                        ...
                        f'dist-to-coast={l1b["dist2coast"]:.0f} km', title_fontsize=legend_fontsize)
        ax.grid()
