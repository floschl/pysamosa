...
THESIS_BASEDIR = Path(
    '/home/schlembach/fastdata/repos/phd-project-flo/thesis/figures/')
...
def gen_multilooked_vary_swh():
    ...
    wf_2 = sm.get_waveform_multilook(
        Pu=Pu,
        Hs=2.0,
        t0_ns=t_0,
        nu=nu,
    ...
    ax.annotate(
        '', xy=(
            x_retracking_point, level_retracking_point), xytext=(
            epoch_ref_gate, level_retracking_point), arrowprops=dict(
                arrowstyle='<|-|>', shrinkA=0, shrinkB=0, color='black'))
    ax.annotate('epoch', xy=(x_retracking_point +
                             (epoch_ref_gate -
                              x_retracking_point) /
                             2, level_retracking_point +
                             0.02), horizontalalignment='center')

    x_pu, y_pu = 25, 1.0
    ax.annotate(
        '', xy=(
            x_pu, 0.0), xytext=(
            x_pu, y_pu), arrowprops=dict(
                arrowstyle='<|-|>', shrinkA=0, shrinkB=0, color='black'))
...
def plot_single_retrackings():
    fig, axs = plt.subplots(
        1, 4, figsize=[
            figwidth_textwidth, figwidth_textwidth / 4], dpi=default_dpi, sharey='row')
    axs = axs.ravel()
    ax_it = iter(axs)
